...
2. **读 `../finaudit-workshop/docs-agent/OPEN-ITEMS.md`** —— 未决事项台账。A 区是阻塞项。
3. 需要历史脉络时读 `../finaudit-workshop/docs-agent/PROGRESS.md`（append-only，最新进展在这）。
   ⚠️ 这两份与失败模式清单都**已移出本仓**（`D-044`）—— 它们是工作记录，不进公开仓。

## 细则索引

| 文件 | 内容 | 什么时候读 |
|---|---|---|
| `../finaudit-workshop/rules/failure-modes.md` | **已经真实犯过的 7 类错**，含识别方法与硬规则。⚠️ **2026-09-11 移出本仓**（`D-044`） | **每次会话开始扫一遍**，尤其在要下「X 没有 Y」这类断言、要声明 flag、要说「读完了」之前 |
...
| `../finaudit-workshop/rules/repo-context.md` | 与外部资料库的关系。⚠️ **2026-09-11 移出本仓**（`D-044`） | 需要外部背景时 |
| `../finaudit-workshop/references/` | 三个外部项目的可借鉴点，**每份标注读到什么程度**。⚠️ **2026-09-11 移出本仓**（`D-043`），在仓外同级目录 | 做架构、PDF 抽取、前端、评测设计之前 |

## 三条最容易违反的

摘自 `../finaudit-workshop/rules/failure-modes.md`，因为它们在一天内各犯了 3 次：
...
- **台账里的「判据」是待办不是已办。** 会话结束前扫一遍 `../finaudit-workshop/docs-agent/OPEN-ITEMS.md` 的 A 区与 D 区（已移出本仓，`D-044`）。
