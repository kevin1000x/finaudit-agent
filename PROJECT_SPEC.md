...
- 两个项目的仓库、语料、结论完全隔离。`data secret` 处在什么状态与本项目无关，本项目的任何结果也不能反向用于该项目。
...
> （失败模式清单 F-6 的实例；该清单已移出本仓，`D-044`）。此处是**同步既有决策，不是作出新决策**。
>
...
- NFR-01：单人可实现。不设每周小时上限；实际投入以会话 changelog 记录（该日志已移出本仓，`D-044`）（D-008，2026-08-10 修订）。
- NFR-02：不引入 SaaS、数据库服务、部署基础设施、生产沙箱。
  ...
  **豁免三（2026-09-06，见 D-021 第三次修订）**：**无状态请求–响应服务**
  （静态前端 + 按请求触发、无常驻状态的 API）**不视为部署基础设施**。
  起因是 D-039 把交付形态升级为正式产品，而产品要为任意公司/年份现算。
  **仍然禁止**（要用须再次修订 D-021）：计划任务 / 后台作业、
  作为记录系统的数据库、多环境机队与值守、用户账号体系。
  缓存只允许**可丢弃缓存**（内容可由公开数据重新推导，清空不丢任何东西）。
  **两句话判越界**：① 有没有进程在没人请求时运行？② 把存储清空会不会永久丢东西？
  任一为「是」即越界。本条用意是防止项目滑向运维工程，
  无状态请求–响应不产生运维班次，计划任务与有状态数据库产生。
- NFR-03：不训练、不微调模型。
...
- **AC-05**（**措辞于 2026-08-27 修订**，操作者授权，见 §9.1）：每条回答产出证据链，**字段齐全率 100%**。
  **本标准不声称证据与答案同源**——它覆盖「证据**缺失**」，覆盖不到「证据**不相关**」。
  同源性由 AC-06 的人工复核承担，**目前无自动判据**。
  原措辞是「产出**完整**证据链」，「完整」读起来像同源性也在内，而实现只保证字段有真值——按 §9.1 引的 harness 先例（把保证削到实现真能满足的强度）改。
...
- **AC-09**（**措辞于 2026-08-27 加强**，操作者授权，见 §9.1；登记册 `L-5`）：执行前静态校验拒绝全部已知危险样本，
  **且每个样本都经过负控制验证**——把该道校验移除后，该样本**确实会被放行**（造回归 → 看红 → 回退，程序见 `rules/commands.md`）。
  原措辞只要求「拒绝」，而一个从来不会被任何样本触发的校验也能满足「拒绝全部」。
  **本条是加强，不是削弱**：它把 `AC-09` 从「样本全绿」提到「闸门被证明在守着东西」。
  **不追溯**——负控制在 Phase 2 静态校验落地时一并做，Phase 1.5 已有产物不重审。
...
| AC-05 | **保证**（**措辞已于 2026-08-27 与覆盖面对齐**） | 齐全率 < 100% 即失败。覆盖面窄于原字面这件事**现已写进 `AC-05` 本身**，不再只活在本表——它只抓「证据**缺失**」，抓不到「证据**不相关**」，见上方「AC-05 的已知盲区」 |
...
| AC-09 | **保证**（**措辞已于 2026-08-27 加强**，登记册 `L-5` 落地） | 某个已知危险样本被放行即不满足；**某个样本在闸门被移除后仍不通过，同样不满足**——那说明它验的不是这道闸门。负控制在 Phase 2 静态校验落地时一并做，**不追溯 Phase 1.5** |
...
⚠️ **本节最初只做分类与后果说明，不改措辞。**
它当时标出 `AC-05` 的覆盖面与 `AC-09` 的强度两处「确实需要改措辞」，
并把裁决留给操作者。

✅ **2026-08-27 已裁决并落地**（操作者授权 agent 判断）：两处措辞都改了，**方向相反**——
`AC-05` **削弱**（把「完整证据链」削到实现真能保证的「字段有真值」，并写明不覆盖同源性），
`AC-09` **加强**（补负控制要求）。
方向相反不是不一致：**诚实化的方向由「措辞与实现哪个更强」决定**，
`AC-05` 是措辞强于实现，`AC-09` 是实现的意图强于措辞。
⇒ 本表从此不再是「唯一记着这两处偏差的地方」——偏差已回到 `AC-xx` 条文本身。
...
- **停滞**：连续两周 changelog 无新条目 → 说明范围估计错误，砍阶段目标。
