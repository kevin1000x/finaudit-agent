...
def test_四个指标不再是NA而是真的算出来了(suite):
    ...
    # 复核一致率与引用可定位率**不由本运行器产出**。
    # 把没做的填成一个数，是评测报告最常见的谎；
    # 而把**别处做完的**数抄进来同样是谎的一种 —— 抄的那一刻它就开始过期。
    #
    # 2026-09-06 改：原先这里断言的是「以 N/A 开头」。H2 做完之后那句话
    # 就成了假的（实验做了，判定是**不成立**），而字符串还写着「02-03 再说」。
    # ⇒ 判据换成真正的不变量：**这一格里不许出现百分比**。
    for name in ("复核一致率", "引用可定位率"):
        assert "%" not in m[name], f"{name} 在运行器里被填成了一个数：{m[name]}"
    assert "VERIFICATION" in m["复核一致率"], "至少要指出真数在哪儿"
