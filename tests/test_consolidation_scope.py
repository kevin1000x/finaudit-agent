...
def test_顺丰2021不用复选框模板_判不可判定而不是返回_False():
    ...
    ## 这一节的措辞 2026-08-31 更正过，原来那句是错的

    原文写「**整节没有复选框模板 —— 六个子项一个都不在**」。**后半句不成立。**
    回到 PDF 逐页读 p250–p253，顺丰那一节的实际形态是：

    ```
    五、合并范围的变更          ← 「五」与「、」之间夹着 U+0007，见下一条测试
      (1) 非同一控制下的企业合并   ← 嘉里物流 145.5 亿、江苏森源、邦威物流、连云港丰泰
      (2) 处置子公司             ← 丰行智图科技，处置收益 8.3 亿
      (3) 其他原因的合并范围变动   ← 现金出资新设子公司若干
    ```

    ⇒ **子项在，而且披露得很详尽。** 对不上的原因有两条，都比原来那句精确：

    1. **没有 `适用 / 不适用` 复选框** —— 这个模板靠「**列出来 = 适用**」表达，
       未列出的三项（同一控制下 / 反向购买 / 其他）即为不适用。
    2. **措辞差一个字**：`非同一控制下**的**企业合并` vs 我们逐字要的
       `非同一控制下企业合并`。

    ## 但判 `undecidable` 仍然是对的，而且是 `D-020` 明写的

    补充节原话：适用性规则**只在采用六项复选框披露模板时成立；
    不采用者记为不可判定、走拒答、不得填 `False`**。
    读数器分辨不出自己在看哪一种模板 —— 在这个模板里「没列出来」确实等于不适用，
    在复选框模板里「读不到」什么都不等于。**两者在数据上长得一样，所以只能拒答。**

    返回 `False` 会让下游读成「本期合并范围没变」，而顺丰本期**换了半张合并报表**。

    ## 真值记在这里，供将来实现该模板时对表

    `business_combination_type = {非同一控制下}`、`consolidation_scope_change = True`。
    **这不是本用例的断言对象** —— 本用例断言的是「判不了」，
    真值写在这里是为了让下一个做这件事的人有个可核对的靶子。
    """
