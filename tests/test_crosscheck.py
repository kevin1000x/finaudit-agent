...
DECIMAL_CONVERSION_SITES = {
    ...
    ("agent/verify.py", "_as_stated"): (
        "把**用户粘进来的那段话里写的那个数**转成 Decimal（去掉千分位逗号）。"
        "🔴 它是**被判定的对象，不是取数输入** —— `D-010` 边界补充写死："
        "用户文本里的数字任何时候都不当事实用，只进比较。与 AKShare 无关。"
        "⚠️ 走 str()/字面量而不是 repr()：输入是人写的十进制串，"
        "从来没经过 float，不存在 repr() 要还原的二进制误差。"
    ),
    ("agent/verify.py", "_judge"): (
        "把 `answer_question` 算出来的那个值化成 Decimal，好与上面那个"
        "用户声明的数做一次带容差的比较。**两侧都不经过 float**："
        "左边源头是 `_as_number`（年报抽取结果的十进制串），右边是 `stated_value`。"
        "转换结果只用于相等判定，**不进任何产物**，与 AKShare 无关。"
    ),
