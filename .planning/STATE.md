...
status: phase-4-done-phase-2-partial   # 2026-09-11 仓库已转 PUBLIC（D-005 修订三，已执行）；/audit/verify 已上线；Phase 2 仍 partial
stopped_at: "✅ **2026-09-09：`h2-02` 重测，停止条件解除。** 一致率 10/15 = 66.7%（> 60% 的停止线，未达 ≥80% 的门槛）、无法判断 0/15 = 0%（此前 86.7%）、自足性 15/15。🔴 **两轮失败的不是同一件事**：`h2-01` 缺的是证据链（读者说不出对错），`D-038` 三节解决了它；`h2-02` 剩下 3 道 FAIL **全是「该答却拒答」**，是能力缺口。**四道 FAIL 盲审全判「对」（错题子集 0/4）—— 一次理由清楚的拒答看起来完全正当，证据页只为做过的事辩护，不显示本来能做得更好。** 复核者换成三个空白上下文子 agent（操作者取消「第二位复核者」门禁），⇒ 读数说明页面自足性，**不说明专业认可**，`AC-06` 那部分仍 `UNVERIFIED`。`N-65`（简称→代码）已做，FAIL 4→3；`N-67`（冻结期望疑似过时）/`N-68`（三处口径定义内容）待操作者。以下是上一轮的原文：🔴 **2026-09-06：H2 判定门不成立，`EVAL_CASES` §4.2 的停止条件已触发（`A-14`）。一致率 2/15 = 13.3%（门槛 ≥80%）、无法判断 13/15 = 86.7%（上限 20%）、判「对」0 题 ⇒ 不得进入 Phase 3。** 转述必带两句：① **不是「答案大量错误」** —— 一致率被 13 道「无法判断」机械拉下来，真实结论是**证据页不足以支撑独立复核**；② 复核者**一道都没判「对」** ⇒ 失败模式是「说不出对错」，不是「被骗」。⇒ `D-038` 已落地（`U-01` 的答案）：证据链补 `G1` 计算输入取值 / `G2` 触发条件的字段取值 / `G3` 指标清单快照，三项**都渲染到人读页面**，三条造回归全部看红。frozen-01 四个数一个没动 —— 补的是给人读的证据，不是能力。**补完 ≠ 通过**：**复核者看过改后样张回复「三节够了」⇒ 修补已验收**，但验收的是「缺口补上了」不是「一致率变好了」——`AC-06` 仍记 `UNVERIFIED`（手上没有改后的读数）。**不为重测单独立项**（操作者 2026-09-06：不要增加重复工作量），什么时候测跟着 `N-46` 或「有第二位复核者」走。另：`D-039` 把交付形态升级为**并入 cninfo 的正式产品**（操作者决定），`D-021` 已随之第三次修订 —— 豁免三**无状态请求–响应服务**，边界两句判据：① 有没有进程在没人请求时运行 ② 清空存储会不会永久丢东西。`D-005` 不变（托管≠发布），`D-014` 不改但**前提已成真**（上线即触发 AGPL §13）。`D-010` 不解除：不收用户上传的财务数据。`02-04-PLAN` 已重写为「真的现算」，**T1/T2 已落地**（`src/service/`，无状态端点 + 覆盖面，零 Web 框架依赖）。**`N-61` 已收口**：`extractor export` + `data/verified/` 人工核对准入清单落地 ⇒ `GET /coverage` 现报**真实 2 行**（茅台 2023 / 万华 2019）**/ 合成 6 行**，「600519 2023 年的资产负债率」= `0.1798432413917914711797119101`，与 `COMPUTED-VALUES.md` 逐位相同。顺丰 2021 有 PDF 但**一个字段都没人核对过**，因此没导出 —— **导出清单等于核对清单**。🔴 新增 `N-63`：**cninfo 现有托管形态（具名 Tunnel 后的常驻 uvicorn）不满足 `D-021` 豁免三判据①**，搭上去要第四次修订 `D-021` ⇒ **挡着上线**，三条路已列出交操作者。`N-62`：万华 3 个 `notes.*` 已核对（§D.19，3/3 一致）**待操作者签署**，已进导出（拦住的代价是整家公司不可用）。**T3 前端已合进 cninfo 站点 `main`**（`a0112c2`）。⚠️ 合之前 `fetch` 才发现站点 UI **在我不知情时被整体重设计**（新增 `DESIGN.md` 作为设计事实源、语义 token、新的 App 外壳），旧版用的裸 `zinc-*` 已不合规 ⇒ **从新 main 重做**，不是把旧版合过来。我曾在过时的本地 main 上执行过一次合并，**没有推**，已撤掉 —— 教训见 `PROGRESS` 第四十六段。🔴 做前端时在浏览器里查出并修掉**服务端两个缺陷**（968 个测试一个都没抓到，只有打开页面才看得见）：① 证据页把**别家公司的年报与 PDF 哈希**报成出处（`_Sources.at()` 找不到行时退回「第一份」，而 `N-61` 之后第一份是一家真实公司）—— 已加两道防线两条回归；② 纯文本证据页里印出 Markdown 星号。**`N-63` 已裁 (a)** ⇒ `D-040`：容器 + 缩到零的按请求托管，**不修订 `D-021`**；连带定死「服务端不引入 Web 框架」（运行时第三方闭包实测只有 PyYAML 一个）。🔴 顺带补上一个真洞：**服务此前完全没有鉴权**（Pages Function 一直在发 Bearer，服务从来不看）——现在绑非回环地址且没设令牌**拒绝启动**，令牌比对走 `hmac.compare_digest`。⚠️ **镜像本身没构建过（本机无 docker），记 `UNVERIFIED`**；验过的是「拷进去的文件够不够」—— 738 KB，只从那些目录跑，覆盖面与答案都对。**`N-62` 已签署**（万华 3 个 `notes.*`，§D.19 3/3）。**T4 平台已定：Hugging Face Spaces**（`D-040` 修订一，操作者 2026-09-07）——与 cninfo 后端同一形态（那个仓库的 `Dockerfile` 通篇写着 HF Spaces / `PORT=7860` / `uid 1000`），免费 Space 闲置休眠、请求唤醒 ⇒ 判据①为「否」，**不需要第四次修订 `D-021`**。🔴 **Space 必须建成 private**：`D-005` 允许对外的是「可访问的链接」不是源码。连带：服务令牌改走 `X-Finaudit-Token`，`Authorization` 留给平台门禁。**仍未定**：Space 的归属与名字（`AUDIT_API_BASE` 填什么）。🔴 新增 `N-64`：**生产路径根本没接模型**（`grep ask_model src/service/*.py` 零命中）——架构做了且刻意（LLM 只许在 `intent` 一层、由调用方注入、客户端零依赖），但线上那一层是纯正则，用户打「茅台」拿不到答案、得打「600519」，即 `A-14` 缺陷一。本仓 **978 passed**，七道门全 exit 0。🔴 仍要操作者：`A-14` 的重测方案、`D-021` **已于 2026-09-06 第三次修订（豁免三：无状态请求–响应服务）**、`N-60` **已结案（并进 cninfo 网站）**、`N-51`、`L-25`、`N-46`、`N-57` 后半"
last_updated: "2026-09-10T00:00:00Z"
last_activity: 2026-09-10
last_activity_desc: "**Phase 4 三条成功标准全部达成，开源决策已作出：`D-005` 修订二 —— 主仓转 PUBLIC**。🔴 **裁定 ≠ 已经转了**：转的动作由操作者本人执行，执行者不代按（不可逆且对外）。裁决前跑了七类安全清扫，范围扩到整仓 + 全部 239 个提交：密钥形状工作树 0、**全历史 0**；数据文件从未进过版本控制；操作者邮箱 0 命中；域名全公开；「雇主/客户数据」命中的全是**禁止它的条文本身**。查出并清掉三项：另一个私有项目的**内部实验状态**（四处）与本机路径、本机绝对路径 4 处、一句个人信息 —— ⚠️ 其中 `OPEN-ITEMS.md` 那一处最难看，**那一条讲的正是「别把本机绝对路径写进产物」，而它的例子把这个毛病又犯了一遍**。明确接受、没处置的两件：① 历史里 5 个提交仍带着私有项目的名字与状态，**不重写历史**（本仓满地是提交哈希引用，而冻结记录的哈希是 `D-012` 的物理保障）；② 69438 行 markdown、126 处 `UNVERIFIED`、324 处 🔴 自我批评会一并公开 —— **这是取舍不是风险**。SC-2 补上了此前**一个都没有**的演示链接 https://cninfo-analyzer-web.pages.dev/#/audit （链路当场验过：`GET /api/audit/coverage` 200，真实 2 行 + 合成 6 行；Space 侧 401，鉴权生效）。**`N-67` 裁：维持 FAIL 不作废** —— 台账那三条理由逐条都要引用系统输出，按 §2.2 属「迁就模型」；另找到一条不看系统输出也成立的（冻结当时「净资产收益率」就不是任何别名，已核 `git show 58c79df:`），**仍不用它**：这已是同一道题第三次找作废理由而动机一次没变。**`N-68` 三条结清，其中两条根本不是口径定义问题是渲染器问题**（`9ce18aa`，不碰 YAML/指纹/version/冻结集）—— `enforced_by` 全仓出现 89 次，**28 次**指向读者找不到的东西，三种机制各不相同。第 3 条按操作者裁「保留别名 + 明写默认口径」，只改 `PROSE_PATHS` 里的散文，`semantic_payload` 实测逐字节不变。🔴 **我的测试自己错了两次，两次同一个形状：用指针本身证明了指针指到的东西存在** —— 判据从整页收紧到那一节之后，红的数量从 2 涨到 19。**`N-69` 裁 (a) 不扩边界，并更正它自己的标题**：归因实测是意图层 1 / 口径层 2，`Q-C3-005` 拒在 `METRIC_NOT_DEFINED`，**根本走不到期间那一步**。🔴 **新增 `N-78`：评测臂不传 `ask_model`** —— `frozen-01` 的分数是「关掉说法归一之后」的分数，不是用户打到的那套系统的分数。**这是 `N-64` 的镜像**：那次修了生产路径，没人回头问评测路径。顺带修掉评测报告里**它自己跑不通的复算命令**（`--suite eval/frozen-01`，实测退出码 2）。门禁 **1107 passed**，六道门全 exit 0（本轮跑三遍全绿）"
progress:
  total_phases: 6
  completed_phases: 4   # Phase 0 / 1 / 1.5 / **4**（Phase 4 于 2026-09-10 三条 SC 全达成；
  # ⚠️ 它依赖的 Phase 2 **并未正式收口** —— 依赖在实质上满足（产物都在且验过），形式上不是
...
完整台账在 `../finaudit-workshop/docs-agent/OPEN-ITEMS.md`（已移出本仓，`D-044`）。
...
1. ~~**T-1 逐条去重归并落地待办**~~ ✅ **完成 2026-08-23**。
   131 次出现归并为 **82 条**去重后的落地动作，清单即登记册 §5（已移出本仓，`D-044`）。
