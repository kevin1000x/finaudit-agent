...
| 测试 | **890 passed**（2026-09-05 每改完一轮文档就从头再跑一次，结果次次一致；耗时区间 148–304s —— 非确定性字段，不是判据） |
