...
——那是人的判断，执行者代答无效。已记为台账 **D-20**，Phase 2 前必须补做。
...
- U-01 证据链字段集未定，等 Phase 2 复核实验数据。**已有三条具体输入**（台账 D-16）
...
- 台账 **D-20**：无人只读 YAML 验证过定义对人可读，D-003 的可复核性前提在这一维度上零证据
...
3. **Phase 1.5 补一条勾稽校验成功标准**（台账 D-13）——cninfo 完全没有这层，要从零建
...
5. **Phase 2 架构预备**：闸门放进**调用签名**（`resolve_scope() -> ScopeSpec | Refusal`），
   不用 waterfall listener 也不用「可选工具」；证据日志的 fail-closed 放在**读入边界**（台账 D-14）
