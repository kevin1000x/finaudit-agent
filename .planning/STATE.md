...
current_phase: "00"
current_phase_name: cninfo 实证结论（产物已齐，待签署）
status: phase-0-awaiting-signoff
stopped_at: "✅ **Phase 0 的产物已齐（2026-09-03），四条 SC 的证据都在，⚠️ 待操作者签署。** cninfo 从「只有怎么用」变成「有一条可复现的实证结论」：`docs/FINDING-01.md` + 五阶段可重跑脚本 + README 首屏重排（cninfo `300bb7b` / `601410c` / `df57da7`，**本地提交未 push**）。**结果是零结果**：n=167，r(Fog, ROA)=+0.025，p=0.75；亏损组 5.3111 vs 盈利组 5.3120，两组差 0.0009 ≈ **0.01 个标准误**。🔴 **但它只能读一半** —— Fog 的变异系数仅 **8.4%**、与 MD&A 长度相关仅 +0.139（p=0.072）、「复杂词」判定退回字符数阈值（常用词表在仓库里不存在）⇒ 写作「在这套测量下没有观察到关系」，**不是**「可读性与业绩无关」。途中修掉两个真缺陷：MD&A 抽取失败率 **64% → 2.3%**（校验器在第一个错候选上放行，而取错的 15–363 字、取对的 12,460–110,874 字，中间是空的）；`extract_mda_section` 找不到时 `return text` 返回整篇年报，**会被当成 MD&A 打分且事后无法察觉**，加占比判据后真的抓到 3 家。顺带实测解掉 AKShare 的 ROA 口径（3 家 2 家逐位相同 ⇒ 平均总资产 + 含少数股东净利润）。⚠️ 两个修复**越过了 `A-12` 的边界**（改了既有模块行为），已如实报告。本仓 **798 passed**，七道门全 exit 0。🔴 仍要操作者：**Phase 0 四条 SC 的签署**、`N-51`（合并及公司合表要不要支持）、`N-20` / `L-25` / `N-46` / `N-44`"
last_updated: "2026-09-03T00:00:00Z"
last_activity: 2026-09-03
last_activity_desc: "**Phase 0 从起草到出结论一轮跑完**，并且这一轮的价值不在那个零结果，在**没有把它读成想要的样子**：做了三个仪器诊断，写明「用一个未证成的仪器得出的零结果，分不开『真的没关系』与『尺子没在量东西』」。同轮否掉操作者「抽取器太死板」的判断（25 份年报实测 24 份逐字命中，覆盖 96%），也否掉我自己「日期前缀该放宽」的提议（该形态 25 份里只出现 1 次）——**两次都是先量再改**。样本损耗如实记、没有补抽；幸存者偏差（25 家退市/ST 掉出样本）**恰好削弱本文要检验的关系**，写进局限"
progress:
  total_phases: 6
  completed_phases: 1
