...
stopped_at: ✅ **Phase 1.5 已收口（2026-08-31）—— 八条 Success Criteria 全部成立。**`SC-2` 由操作者签 `PASS`（29 / 29 ≥ 24，作废 1 条 `notes.reporting_period_months` 按 `D-026` 移出分母），`SC-8` 08-30 签 `PASS`。29 个字段逐字对照茅台 2023 年报原文，**零不一致**；读数走 `pdfplumber` 原始版面文本，不经 `locate` / `mapping` 任何一层。⚠️ **`SC-2` / `SC-8` 两个 `PASS` 的效力有一条不随签署消失的限定**（`VERIFICATION.md` §C.13）：核对由**执行者**执行、操作者审阅接受 —— **代码路径级独立，不是执行者级独立**。🔴 **下一个动作：把三支 `kind` 接进 `pipeline` 的批次产出**（`N-43` 判据 3）。**先回答设计问题再动手**：`ExtractionRecord` 的 `column_header` / `unit` / `currency` 对一个复选框字段意味着什么 —— 编一个值填进去就是 `F-2`。在接上之前 **6 个指标的 `unevaluable` 现状不变**。其余在册：`N-44`（`D-030` 余量按天下行，约 7 天后跌破，处置不是再下调）、`N-41` 判据 3 的返工未做、`N-40` 判据 2 未做、复核余下 4 条 findings（`RV-7`/`RV-3`/`RV-8`/`RV-9`）未修、`frozen-02` 待办、`N-42` 未做的一半、Phase 0 未开始
last_updated: "2026-08-31T00:00:00Z"
last_activity: 2026-08-31
last_activity_desc: **Phase 1.5 收口 —— `SC-2` 签 `PASS`，29 个字段零不一致**。本轮补核剩余 10 个字段（§D.17），10/10 一致。四处是真正的辨析测试：`营业成本` vs `二、营业总成本`（差 4 倍、标签前缀包含）、`34.19` vs 干扰项 `34.20`（差 0.01、标签整体包含 —— `label_fragments` 的「逐字整行相等、不是子串」这条设计第一次被真实数据检验，写成子串匹配会静默取错）、`非经常性损益合计` 跨页算术互证（p5 两行相减 = p6 合计，逐字相同）、`加权平均净资产收益率` 二次互证（34.19 − 30.26 = 版面自印的「增加3.93个百分点」）。另：`cfs.cash_paid_for_fixed_...` 是 `A-6` 失效模式 1 的真实复现且三行缝合正确接回；`notes.business_combination_type = ∅` 不是「没抽到」—— 按 `D-028` 空集即表示本期无企业合并
progress:
  total_phases: 6
  completed_phases: 1
