...
### pdfplumber 单库实测（2026-08-16，A-4 判据）

**结果：行项目命中 10/12 ≥ 9/12，A-4 判据达成，D-014 的反转条件未触发。**

同一份 PDF（贵州茅台 2023 年报，143 页 3.4 MB，`orgId=gssh0600519`），
`pdfplumber 0.11.5`，`extract_words(x_tolerance=1.5, y_tolerance=1.5)` + `round(top/3.0)` 聚簇。

⚠️ **口径说明**：上一轮的「12 个目标行项目」具体是哪 12 个**没有记录下来**。
本次重新定义为 20 份定义实际需要的 12 个 `bs.*` 字段，
因此这是**重新标定的基线，不是与上一轮逐项可比的复跑**。

```
[命中] bs.total_assets                          p59  ['272,699,660,092.25', '254,500,826,096.02']
[命中] bs.total_liabilities                     p60  ['49,043,190,797.43',  '49,562,744,832.16']
[未中] bs.total_equity                               ← 折行，见坑五
[未中] bs.minority_interests                         ← 窗口太窄，实为 p61 有值 7,987,897,687.39
[命中] bs.total_current_assets                  p59  值✓
[命中] bs.total_current_liabilities_period_end  p60  值✓
[命中] bs.inventory / accounts_receivable / accounts_payable  值✓（但混入附注号，见坑三）
[命中] bs.short_term_borrowings                 p59  **行在但格子空**
[命中] bs.long_term_borrowings                  p60  **行在但格子空**
[命中] bs.bonds_payable                         p60  **行在但格子空**
```

两个未命中的**原因不同**，不要混为一谈：
`minority_interests` 只是我的翻页窗口取了 3 页没覆盖到，单独查 p61 有值、格式正常；
`total_equity` 是**真实的方法缺陷**（坑五）。修掉坑五后可达 11/12 已验证可抽。

#### 表的真实版式（这段比命中数更有用）

```
p58 末尾  审计报告签字 → 「二、财务报表」→ 合并资产负债表
          编制单位:贵州茅台酒股份有限公司
          单位:元 | 币种:人民币                       ← 表头只在这一页
          项目 | 附注 | 2023年12月31日 | 2022年12月31日  ← 列名只在这一页
          货币资金 | 1 | 69,070,136,376.12 | 58,274,318,733.23
p59–p60   续页，**表头与列名都不重复**
p61       所有者权益段落 → 紧接着 **母公司资产负债表** 开始
p62+      母公司表正文
```

#### 坑三：**附注编号列会被当成数值**

列结构是 `项目 | 附注 | 本期 | 上期`，附注号在第 2 列：

```
应收账款 | 5 | 60,373,410.41 | 20,937,144.00
存货     | 9 | 46,435,185,061.53 | ...
```

按「行首是标签、其余是数字」取值，会把 `5` / `9` 当成本期数。
**必须按列位置剔除附注列，不能按值形状判断**——附注号与小额科目在正则上不可区分。
好在列名行给出了列语义，但它**只在表的首页**（见下一坑）。

#### 坑四：**续页不重复表头，母公司表紧接合并表**

- 从任一续页单独开始抽取，会**同时丢掉单位、币种与列定义**；
- p61 一页之内从合并表切到母公司表。**宽页窗口会静默混入母公司数**：
  合并资产总计 `272,699,660,092.25`，母公司资产总计 `171,584,366,864.08`。
  这正是 `parent_only_scope` flag 的用武之地——**它此前零引用，现在有实证承载场景了**。

#### 坑五：**长标签在单元格内折行，数值落在两行标签之间**

y 聚簇法在这里会失效。原样贴出重组结果：

```
归属于母公司所有者权益
215,668,571,607.43 | 197,480,041,239.46      ← 数值单独成一行
（或股东权益）合计
```

`所有者权益（或股东权益）合计` 与 `负债和所有者权益（或股东权益）总计` 同样如此。
按「行首是标签」的规则，这些行**永远匹配不上**——`bs.total_equity` 就是这么丢的。
数字垂直居中于合并单元格，而标签占两行，于是数字自成一个 y 桶。
**处理方式应当基于 x 坐标分列（标签列 / 附注列 / 数值列），而不是基于行首**。

#### 顺带确认的两件事

- **`单位:元 | 币种:人民币` 就印在表头** —— 台账 B-3 说的承载字段确认存在且抽得到，
  `unit_scale_mismatch` 与 `currency_mismatch` 两条 flag 在 Phase 1.5 后可真正启用。
- **「空格子 ≠ 缺失」在 pdfplumber 上独立复现**：短期借款 / 长期借款 / 应付债券
  三行印在报表上、值的格子是空的（茅台确实无有息负债）。A-2 的实证与抽取库无关。
