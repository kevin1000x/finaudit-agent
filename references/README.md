...
3. **每份产物必须指出落地点。**（2026-08-22 新增，因为量过一次账，发现沉淀严重不均）
   ...
   **（2026-08-24 补）落地之后必须回来改这个标注，并写上登记册编号 `L-nn`。**
   标注的可信度上限 = 它最后一次被审视的时间。只改「未落地 → 已落地」而不写编号，
   从 `references` 这一侧就追不回登记册——**第六道门的 R5 查的就是这个编号**，
   没有它 R5 会报红。理由不是形式主义：2026-08-24 一天之内，
   「落地后必须同时回标」这条规则刚写下就被违反两次（五条落地、零条回标）。

4. **区分「他们怎么做」与「我们要怎么做」。** 后者必须给理由，尤其是**刻意不借**的地方——
   偏离通用做法而不写理由，下一个人会以为是疏漏然后「修好」它。
5. **引用必须自带可独立核验的凭据。**（2026-08-27 新增，登记册 `L-7`）

   **(a) 引散文时必须同时贴一段可 `grep` 的原文字面量，不能只写行号。**
   实证出自本文件夹自己：`hello-agents-ch09-context.md` §12.5 对正文 235 处
   `ch9:` 引用做了全量行号复核，改正 10 处偏移 —— 其中 **7 处落在 `ch9` 的前 140 行**，
   而那 140 行几乎全是散文；`ch9:140` 之后**有代码**的段落命中率接近 100%。
   差别不在细心程度，在**取位置的方式**：代码行有唯一可搜的字面量
   （`break`、`text[:max_chars]`、`"note_id": note['note_id']`），写的时候是搜出来的；
   散文段只能靠「读到第几段」记位置，偏移全发生在这里。
   反过来说，这 10 处能被查出来，靠的正是那些**带了原文的**条目——它们一条都没错。

   **理由与 `ARCHITECTURE.md` §8.5 同源**：一个无法被独立核验的指针（只有行号、没有内容）
   等于没有指针，它的正确性完全寄托在「写的人当时记对了」，
   而那恰恰是复核要检验的东西。**这条规则和证据链是同一条原理，只是作用在读书笔记上。**

   **(b) 本文件夹里出现的任何「运行输出」必须附可复现命令，否则标 `UNVERIFIED`。**
   出处 `hello-agents-ch09-context.md` C9-B9：书稿五处「运行效果展示」
   （`ch9:655-687`、`:1490-1550`、`:2469-2739`）经核对**都跑不出来**，含一处安全断言
   （`ch9:1690`）。**看起来详实的假输出比没有更危险**——读者会拿它当基线。
   ⚠️ 这半条**在登记册里没有对应的 `L-nn` 行**（归并时并进了相邻条目），
   与 (a) 同落一处是因为它们约束的是同一件事：**引用的凭据形态**。
