...
| # | 条目 | 出处 | 落地点 |
|---|---|---|---|
| **S6-1** | **把「这个字段怎么被合并」写进字段类型**，而不是指望每个写入方记得 append。`Annotated[list, add_messages]` 让 reducer 成为 schema 的一部分 | LangGraph，`Dialogue_System.py:24`；§5.2 理由一 | **未落地。** 应新增一条决策（`DECISIONS.md`，紧接 D-019 的编号）：*证据链字段的合并语义写进 schema，禁止由写入方自行决定覆盖还是追加*。在此之前先记入 `docs/agent/OPEN-ITEMS.md` N 区。⇒ **已落地 2026-08-27**（登记册 **`L-44`**，wave 1–3 实现）：`record.ExtractionRecord.MERGE_SEMANTICS`（ClassVar，schema 层声明合并语义，不由写入方各写各的） |
...
| **S6-7** | **书给的调试三分层**：① 节点内部逻辑错 ② 节点间传递的状态数据异变 ③ 边跳转条件失误。这是全章最有用的一句，且它正好点名了书自己在优势里推荐过的两个写法 | `ch6:1286`；§5.7 末行 | ~~**未落地。**~~ → **已落地 2026-08-27**（登记册 **`L-51`**，`EVAL_CASES` **§5.4**）。照原文的定位落成**对照而不是替换**：三分层按「错在节点里还是节点之间」切，本项目四层按「错在哪一环」切，两者正交，用处是做一次覆盖自检。自检结果写进了该节：**四层都表达得了，但第 ② 类（口径传递错）落点最含糊**——它在四层里只能记成口径层，而它与「一开始就选错口径」是两种不同的修法。**本轮不新增第五层**（会动 `AC-08` 的归因覆盖率口径，属规格变更），改为写下一条**演化触发条件**：Phase 2 复核实验回来后若口径层失败里超过一半是传递错，这一层该拆。以下为原文，保留不改：`EVAL_CASES.md §5 失败归因分层`已存在但分层维度不同。这三分法应作为**归因分层的一次外部对照**补进 `EVAL_CASES.md §5`——本项目的对应物是「抽取错 / 口径传递错 / 闸门判定错」 |
...
| **X6-2** | **用类型注解假装有契约，但不跑 checker** | LangGraph Demo 三个节点全部注解 `-> SearchState`，实际返回部分字段 dict（`:42/:80/:132`）；`code/chapter6/` 零测试、零 mypy、零 pyproject；§5.2 代价一 | **比没有类型更危险**：它制造"我们有类型"的错觉，于是运行时校验被省掉。注解一旦不被执行就退化成注释，而注释可以撒谎且没人发现。注意书稿正文这里反而是对的（`ch7` 侧同理）——**错在仓库，不在理念**，说明这类腐烂是"写完没人查"造成的 | ~~**未落地（本轮新发现的缺口）。**~~ → **部分落地 2026-08-27**（登记册 **`L-25`**，`rules/commands.md` §「已识别、但尚未机械化的门禁缺口」(1)，与 ch07 的 `C7-X13` / `C7-X9` 合成一条）。**落的是约束、不是门禁**：三条现在就成立的约束（注解必须能被 checker 验 / 公开接口声明了就必须有消费者 / 跨模块调用关键字传参）已写死，**mypy / pyright 进门禁命令这一半没做**，该节开头明写「不许把本节读成已经有检查了」。以下为原文，保留不改：**D-015** 已经用对了同一个机制（「依赖方向单向且**由测试强制**」），但它只覆盖依赖方向。「类型注解必须由 checker 执行」缺一条**仓库级门禁**：应落到 `rules/commands.md` 的门禁一节（加 mypy/pyright 到冻结校验同一批命令里），Phase 1.5 起生效 |
...
| **X6-7** | **把派生数据物化成多个并列字段，靠手工同步** | AgentScope 的存活状态同时存在于 `alive_players` / 五个阵营子列表 / `roles` / `players`；`update_alive_players` 必须逐一剔除同一个死者（`main_cn.py:258-269`），漏一个 = 幽灵玩家；§3.6 | 漏改不报错，只产生行为漂移。这与 X6-6 是一对：一个是"一个字段两种意思"，一个是"一个意思多个字段" | **未落地。** 本项目的风险面在映射表与抽取记录之间（同一个科目的标签、命名空间、页锚点可能各存一份）。应落到 **D-016 的补充节**，写明*派生集合不得与权威表并列存储，只能在读入时按需物化*。⇒ **已落地 2026-08-27**（登记册 **`L-37`**，wave 1–3 实现）：`mapping.declared_field_ids()` 运行时遍历 `metrics/` 物化，仓库里没有第二份清单；含一条「改 `metrics/` 它必须跟着变」的证明性用例 |
...
**该刻意不借 10 条**：已落地 **7**（X6-3…X6-6 / X6-8 / X6-10 / **X6-1**），未落地 **3**
（**X6-1 = 登记册 `L-19`**，`ARCHITECTURE` §8.3 的 `completed_degraded`，提交 `10d2927`）（X6-2 / X6-7 / X6-9）。
...
> **→ 部分落地 2026-08-27**（登记册 **`L-26`**，`rules/commands.md`
> §「已识别、但尚未机械化的门禁缺口」(2)）。**落的是约束**：文档里写死字段名 / 类名 /
> 枚举值必须是从代码复制出来的，改代码时同步改文档；
> **`verify-doc-symbol` 那道门没做**。原文建议的「与 X6-2 合并成一条待办」**没有照办**
> ——登记册把它们分成 `L-25` / `L-26` 两行，两行都要各自可追，合并会让其中一条消失。
> 落地时按 `deepseek-harness-docs-part2.md` HD2-13 补了对方的做法
> （`development.md:163-171` 的 `{doc, symbol, source}` 清单 + 解析器 1:1 校验），
> 并写明本项目要用 Python `ast` 而不是正则。
