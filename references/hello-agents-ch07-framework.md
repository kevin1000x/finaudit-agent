...
| **X-9** | **位置传参 + 签名不统一 ⇒ 静默参数错绑** | §3.3。`super().__init__(name, llm, system_prompt, config)` 把 `system_prompt` 绑到父类的 `tool_registry` 形参上；因为 `MyReActAgent.run` 从不读 `self.system_prompt`，**这个错误永远不会表现出来** | **一处静默的参数错绑，在一个"输出看起来正常"的系统里可以永远不被发现。** 它同时命中三件事：签名不统一 + 位置传参 + 子类不读被污染的字段。可核查性要求的不是"不出这种错"，而是"出了能被机器发现" | ~~**未落地。**~~ → **部分落地 2026-08-27**（登记册 **`L-25`**，与 `X-13`、ch06 的 `X6-2` 合成一条，落在 `rules/commands.md` §「已识别、但尚未机械化的门禁缺口」(1)）。**「跨模块调用一律关键字传参」这条约束已落，lint 门禁未做。** 以下为原文，保留不改：构造上的答案是**关键字传参**（lint 可查）＋ S-3 的**运行时记录实际生效参数**。应与 X-13 合并成一条仓库级门禁，落到 `rules/commands.md` 的门禁一节 |
...
| **X-11** | **宣布成功 + 返回失败**（终态不区分正常与降级） | §4.4(b)(c)。`ToolChain.execute` 步骤间零错误检查，错误串被当正常结果存进 `context` 并作为下一步输入；书给的示例链 `create_research_chain()` **确定性输出**是：控制台打印两次「✅ 步骤 N 完成」+ 一次「🎉 工具链执行完成」，返回值是「计算失败，请检查表达式格式」。而 `AsyncToolExecutor` 的 `gather` 没有 `return_exceptions`——但这不重要，因为异常早已被转成字符串，`gather` 永远看不到 | 这是「降级/失败被写成与成功相同的终态」的第二种形态（第一种是 ch06 的 `step="completed"`）。**书是把它作为正面示例给出的** | **已落地（`ARCHITECTURE.md §8.3` 补充节，提交 `10d2927`；与本仓 ch06 的 `X6-1` 是同一条，两章各自独立命中）。** **回标（2026-08-23，T-5）**：标注此前未回改，属 T-1 查出的 4 条 `STALE` 之一。以下为原文，保留不改： `ARCHITECTURE.md §8.3` 只规定了**拒绝**事件的结构化理由码，**未覆盖「降级成功」**。应在 `ARCHITECTURE.md §8` 新增：*成功终态必须区分 `completed` 与 `completed_degraded{cause: 封闭枚举}`，`cause` 不得因取值为空而消失*。这是 **AC-05** 与 **AC-08**（失败归因覆盖率 100%，不允许"未知"）的前置条件 |
...
| **X-13** | **死抽象：声明了但零消费** | §2.2（`Config` 全包零消费：`from_env` / `max_history_length` / `config.temperature` 命中均为"仅自身声明"）、§4.1(b)（`validate_parameters` 全包零调用、`.to_dict()` 命中 0）、§5.2(c)（`setup_logger` 零消费） | 死抽象比没有抽象更糟：它让读者与复核者以为某件事已经在做了。`max_history_length` 声明了历史上限而**没有任何地方裁剪历史**，是这条最干净的例子 | ~~**未落地。**~~ → **部分落地 2026-08-27**（登记册 **`L-25`**，与 `X-9`、ch06 的 `X6-2` 合成一条，落在 `rules/commands.md` §「已识别、但尚未机械化的门禁缺口」(1)）。**「公开接口声明了就必须有消费者，没有消费者的先别声明」这条约束已落，lint / 覆盖率门禁未做。** 以下为原文，保留不改：应与 X-9 合并成一条**仓库级门禁**，落到 `rules/commands.md`：*声明了但零调用的公开接口必须由 lint / 覆盖率检查发现*。与本仓 ch06 的 **X6-2**（有类型注解但不跑 checker）是同一类门禁缺口，**两章各自命中** |

### 7.3 计数

**该借鉴 9 条**：已落地 3（S-4 / S-6 / S-7），未落地 6（S-1 / S-2 / S-3 / S-5 / S-8 / S-9）。
**该刻意不借 13 条**：已落地 **8**（X-3…X-8 / X-12 / **X-11**），未落地 **5**（X-1 / X-2 / X-9 / X-10 / X-13）。
**合计 22 条，11 已落地 / 11 未落地。**

> **计数已于 2026-08-23（T-5）同步**：X-11 由「未落地」改判为已落地（`10d2927`，与 ch06 的 X6-1 同一条）。
