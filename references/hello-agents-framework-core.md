...
**落地点：~~未落地~~ → 已落地 2026-08-27**（登记册 **`L-65`**，
`rules/commands.md` 新增「本项目的测试写法约定」一节）。
三条照原文写死，并把第 3 条接到本仓两条验收标准上：`AC-02`（口径缺失时拒答）与
`AC-09`（静态校验拒绝危险样本）都写成 `pytest.raises` + **断言精确的理由码**，
**不写「返回值非空」**——非空可以由一句错误字符串满足。
第 2 条另与第六道门的 R3、`rules/pitfalls.md` 第 18 条对接（形状断言测的是比较器）。
以下为原文，保留不改：
...
**落地点：~~已落地（`D-015` 依赖测试强制），但需要补一条元判据~~ → 元判据已落地 2026-08-27**
（登记册 **`L-32`**，`rules/commands.md` 新增「门禁断言的元判据：说不出它什么时候会红，就删掉」一节）。
落地时写清了**它与第六道门 R2 的分界**：R2 是这条元判据机械化的一半
（每道门禁必须具名声明一个自己会红的负控制），
**但 R2 保证不了「这条断言本身答得出那句话」**——剩下那一半没有机械判据。
并规定说不出来时只有两个处置：删掉，或按「造回归 → 看红 → 回退」把那个场景真的造出来。
以下为原文，保留不改：
