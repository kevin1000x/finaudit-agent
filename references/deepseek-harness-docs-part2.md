...
> **→ 已落地 2026-08-27**（登记册 **`L-84`**，见本文件 §12 表第 21 行的回标）。
...
| # | 启示 | 出处 | **该落到哪里** |
|---|---|---|---|
| 6 | **「假闸门」有机械化的识别规则**：非空但从不调用 reporter 的检查 = 伪造；不许断言「service/method 存在」这类恒真的东西；确实没有可检查项时必须写空实现 + 该模块特有的、以固定前缀开头的理由，模板化措辞由脚本拒绝 | `invariants.md:59`、`:5` | ~~**未落地。**~~ → **已落地 2026-08-24**（`LANDING-BACKLOG` L-4）。落成**第六道门** `scripts/check_gates.py`：R1 拒绝无可失败点的测试、R3 拒绝恒真断言，F-2 增补了「执行机制」一节。两条判据都实现了——「诚实的没有」用 `NO-ASSERT-BY-DESIGN:` 前缀 + 理由全仓唯一。**局限：只覆盖测试侧，flag 的 trigger 侧仍无门禁。** |
| 7 | **「闸门只有在回归真的被它挡住时才算闸门」**，且附三步验证程序：故意引入违规 → 看它变红 → 回退 | `testing.md:34`；实际执行的样子见 `postmortem/0001:104`「Verified it fails when `export default apply` is restored」 | ~~**未落地。**~~ → **已落地 2026-08-24**（`LANDING-BACKLOG` L-5）。三步程序已写进 `rules/commands.md`，并**实跑了一次**——第一次跑砸了（造回归的脚本把文件切成语法错误，门禁红的原因不是被查的那件事），因此程序里加了「第 2 步必须连带确认红的原因是对的」。R2 的具名负控制注册表是它的机械化那一半。~~**AC-09 的强化仍未做。**~~ → **2026-08-27 已做**（`L-5` 收尾）：`PROJECT_SPEC` `AC-09` 条文补上「每个样本经负控制验证——移除该校验后该样本确实被放行」，`ROADMAP` Phase 2 第 5 条同步。 原文其余部分：落 F-2 与 F-8，并强化 **AC-09**：现在只要求「拒绝全部已知危险样本」，须补「先证明这些样本在闸门缺失时确实会通过」。同时进 `EVAL_CASES §3.3` 的失败归因分层：要能区分「闸门拒对了」与「闸门根本不可能触发」 |
| 8 | **生成一张「产生点 / 消费点」矩阵，让断线自己显形**，而不是写规则要求下游记得接线 | `event-producer-consumer.md:76`（从 TS Program 解析生成）+ 本轮统计出 56 条事件里 7 条零监听 | **未落地。** 落 `ARCHITECTURE §8.5` 新增「证据指针矩阵」一节 + `rules/commands.md` 增加生成命令（Python `ast` 解析 `field_id` / `anchor_page` 的产生与消费点）。**§8.5 目前的写法「必须被下游强制消费」是一条纪律，可以被违反而不被发现；矩阵是可检查的** ／ **回标 2026-08-26：约束已落地**（登记册 **`L-6`**，`ARCHITECTURE` **§8.5.3**）。落地时把「纪律可被违反而不被发现」这句**换成了本仓自己的实例**——§8.5 的依据栏此刻已记了四个「指针被生产、无人消费」的实例，本条要防的正是同一件事。**生成命令仍未实现**，理由写在 §8.5.3 的尾注：抽取器还没产出真实字段，现在写会写出一个查不到东西的空壳（那是 F-2 本身） |
| 9 | **留痕提交失败 ⇒ 结论作废**（上游那一半） | `approval.md:122-126`「returning an unlogged decision would violate the pair」+ `:117-119`「an idle ask rejects before appending anything」 | ~~**未落地。**~~ → **已落地 2026-08-23**（`LANDING-BACKLOG` L-1，`ARCHITECTURE` §8.5.1）。上游那一半已补，含提交点分界线。原文：§8.5 当时只约束下游消费，上游「写不进就不算数」完全没写。要连带明确一个提交点：提交前失败 ⇒ 整体失败，提交后观察者失败 ⇒ 被 contain，不得反噬 |
| 10（= 登记册 **`L-47`**，**已落地 2026-08-29**：`RefusalCode.UNAVAILABLE` 已进代码，枚举由 7 支增至 **9 支**（另一支是 `D-025` 的 `RECONCILIATION_FAILED`），`extractor/download.py` / `crosscheck.py` 共 10 余处真实构造点） | **「基础设施不可用」必须是拒答枚举的一支，不是异常** | `approval.md:21`、`:28`（`unavailable` 是四支之一）、`:119-122`（rogue 返回值归一化为 `unavailable`） | **决策已落地（D-022 决策一，提交 `c6d5c6e`），实现待 Phase 1.5。** ~~**回标（2026-08-23，T-5）**：已核 `RefusalCode` 目前仍为 7 支，`UNAVAILABLE` 尚未进代码——**这不是 `STALE`，是「决策已落、实现待阶段」**~~ → **该状态已于 2026-08-29 结束**：`len(RefusalCode) == 9`，`UNAVAILABLE` 与 `RECONCILIATION_FAILED` 都在代码里且有真实构造点，见 `LANDING-BACKLOG` L-47。另：原文担心的「D-012 冻结校验与『枚举恰为 7 个』的测试会挡住这个改动」，D-022 已实测推翻——`len(RefusalCode)` / `list(RefusalCode)` 全仓零命中，不存在那条锁死测试。以下为原文，保留不改：**已核实缺口**：`grep -rn "UNAVAILABLE\|NOT_AVAILABLE\|SOURCE_MISSING" --include="*.py" src/` → **0 命中**。现有 7 个 `RefusalCode` 全是「口径/数据本身有问题」，没有一支表示「口径服务或数据源不可达」。落 `src/semantic_layer/resolve.py` 的 `RefusalCode`（**注意：D-012 的冻结校验与「枚举恰为 7 个」的测试会挡住这个改动，须先走决策变更**）+ `ARCHITECTURE §8.2` |
...
| 13 | **文档里粘贴的类型/枚举，用 gate 与源码焊死**（清单 `{doc, symbol, source}` + 解析器提取 + 1:1 对应校验） | `development.md:163-171`（`ts type-equiv` + `verify-type-equiv`） | ~~**未落地。**~~ → **已落地 2026-08-27**（登记册 **`L-8`**，落成两处配对使用）：**`DECISIONS.md` `D-012` 新增「局限」小节**——写明 SHA-256 的粒度只到产物级，能证明「这份文件一字未改」，证明不了「文件里描述的东西与代码一致」；⇒ **凡引用了代码中符号的文档段落，不得以「已通过 D-012 的哈希校验」作为它仍然正确的理由**。缺口的门禁侧记在 **`rules/commands.md` §「已识别、但尚未机械化的门禁缺口」(2)**（`verify-doc-symbol` 未实现，且写明要用 Python `ast` 而非正则）。**本条预言的那个缺口已经兑现**：写这条时担心的是「文档说 7 项、代码 8 项」，此后 `D-022` 把 `RefusalCode` 扩到 8 支、`D-025` 又扩到 9 支。以下为原文，保留不改：落 `rules/commands.md` 新增 `verify-doc-symbol`，用 Python `ast` 而非正则。D-012 的 SHA-256 只覆盖**文件级**，覆盖不了「文档里说 `RefusalCode` 是 7 项、代码里已经 8 项」这个粒度——而第 10 条恰好就要动这个枚举 |
...
| 15 | **不调模型的主流程，必须有无密钥、走真实入口的 CI 测试** | `postmortem/0001:112`「When the headline operation does not call the model, that test needs no API key — so it belongs in CI, not behind a key gate」 | ~~**未落地。**~~ → **已落地 2026-08-27**（登记册 **`L-27`**，`rules/commands.md` §「已识别、但尚未机械化的门禁缺口」(3)）。落地时把它写成**一条现在就成立、且约束未来的判据**：口径解析 / 拒答判定 / 证据链装配的测试**不得**以「需要密钥」为由排除在 `gates.yml` 之外；某条测试确实需要密钥就说明它测的不是主流程，应当拆开。现状是六道门全部无密钥，本条当前成立——**它防的是接了模型之后顺手把主流程测试挪到密钥门后面**。以下为原文，保留不改：本项目的口径解析、拒答判定、证据链装配**全部不调模型**，因此都属于这一类。落 `rules/commands.md` 与 `gates.yml`（D-021 已建 CI 门禁，此条是给它加内容） |
| 16 | **正交事实各自独立上报，不许把一个塞进另一个的分支里** | `defensive-patterns.md:7-9`（`timedOut` / `signal` / `exitCode`） | **未落地。**「取到数了」「口径已确认」「数据源新鲜」是三个正交事实，不能嵌套。落 `ARCHITECTURE §8.3`（终态三分已经是这个方向，但只分了终态，没分这三个正交维度）。⇒ **已落地 2026-08-27**（登记册 **`L-14`**，wave 1 实现）：`record.ExtractionRecord` 的 `retrieval` / `basis_confirmation` / `source_freshness` 是**三个独立字段**，互不嵌套；有一条「三个正交事实是三个独立字段」的用例 |
...
| 18 | **不能证明因果时，证据链只能写「同一区间内发生」，不能写成因果归因** | `defensive-patterns.md:15-17`（`describe any selected output as interval-wide rather than causally attributed to that message`） | ~~**未落地。**~~ → **已落地 2026-08-26**（登记册 **`L-15`**，`ARCHITECTURE` **§8.5.6**）。以下为原文，保留不改：**未落地。** 本项目证据链默认「结论 ← 依据」是因果的；并发取数时该默认不成立。落 `ARCHITECTURE §8.5`。**落地补记**：2026-08-25 的沉淀审计确认本条**不属 Phase 1.5**（该阶段单公司单年顺序抽取，并发场景不成立），因此 §8.5.6 写成「条文先立、Phase 2 生效」，并写明先立的理由——并发是被「顺手加个线程池」引入的，那一刻没人会回头翻架构文档 |
| 19 | **审计事件 log-only，不进模型上下文** | `approval.md:88` | ~~**未落地。**~~ → **已落地 2026-08-26**（登记册 **`L-16`**，`ARCHITECTURE` **§8.5.4**）。落地时把「破坏复核的独立性」展开成了可陈述的理由：AC-06 要求复核者仅凭证据链独立判断，证据若参与了产生答案的那次采样，**一致就不构成印证**。并配套写了「`[Evidence]` 与 `[Output]` 分区不参与预算裁剪」（见 §8.7）。以下为原文，保留不改：**未落地。** 落 `ARCHITECTURE §8.5`。防的是「把证据链塞回 prompt 当上下文」——既费 token，又让证据内容影响模型输出，破坏复核的独立性 |
| 20 | **关掉一个检查 ≠ 该检查从未存在**：被过滤掉的注册仍然占住名字 | `invariants.md:55` | ~~**未落地。**~~ → **已落地 2026-08-26**（登记册 **`L-17`**，`ARCHITECTURE` **§8.5.5**）。落地时按 `references/README.md` 第 4 条**写清了不借的那一半**：借的是「被关掉的东西仍然留名」，不是「口径闸门可以被关掉」——本项目的闸门不可替换、不可绕过。以下为原文，保留不改：**未落地。** 落 `ARCHITECTURE §8.5`。若将来做可配置关闭的口径校验，被关掉那条的标识必须仍在日志里，否则「关掉 A」与「A 从未注册」分不开 |
| 21 | **`FIXME / TODO / XXX` 三级，其中 `XXX` 明写「可能永远不做、无承诺」** | `development.md:155-161` | ~~**未落地。**~~ → **已落地 2026-08-27**（登记册 **`L-84`**，`docs/agent/OPEN-ITEMS.md` 新增 **X 区「无承诺（可能永远不做）」**）。落地时补了原文没有的三样：① **它与台账开头「没有判据的条目不许进这个表」这条规则正面冲突，冲突已就地解决**——X 区是那条规则的唯一例外，用「重新考虑的触发条件」代替「判据」，写不出触发条件的不许进本区、直接关闭进 E 区；② **「放弃了什么」也是必填**，与同日落进 `D-009` 判据补充的归档模板那一节同形态；③ **不新起编号命名空间**，条目从原区移进来保留原编号，否则交叉引用门禁的定义点正则匹配不到、已写出去的引用会失效。⚠️ **本区当前为空**且写明了理由：把某一条现有条目判为「可能永远不做」是操作者的判断，不是可以顺手做掉的整理动作；条文里另写了「空着也不许拿来充数」。以下为原文，保留不改：落 `docs/agent/OPEN-ITEMS.md`。现在 A 区（阻塞）/ D 区（判据）两分，缺「可能永远不做」这一档——导致所有条目看起来都像迟早要做 |
