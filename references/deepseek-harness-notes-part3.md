...
| # | 借鉴什么 | 出处 | 落地点 |
|---|---|---|---|
| C-A1 | **抽样/排序/截断的口径必须随结果一起呈现**：超上限时页脚明说「这是跨条目抽样而不是修改时间头部」，并报告触及了多少个顶层条目；只要 spill 成功，**完整排序列表保留在 artifact 里** | `B-10:15-17` | ~~**`未落地`**~~ → **部分落地 2026-08-23**（`LANDING-BACKLOG` L-2，`ARCHITECTURE` §8.5.2）：证据指针必须携带**取数口径三元组**（抽样方式 / 排序键 / 截断位置）且被下游强制消费——**已落**；~~`EVAL_CASES §3.3` 那条判据（引用截断页而未声明口径记失败）——**仍未落**~~ → **已落地 2026-08-29**（登记册 **`L-2`**，`01.5-06` Task 2）：`EVAL_CASES §3.3` 的 `J-7` 已有机械检查 —— `tests/test_evidence_ids.py` 逐项对 `sampling` / `order_key` / `truncation` 各造一次空值，每次都拉低 `AC-05` 齐全率并在 `gaps` 里点名是哪一条；「是否构成对 `D-019` 的扩展」这个悬而未决的问题已由 **`D-023`** 裁决为**是**（显式扩展） |
| C-A2 | **同一事实的多个结构表示是「可静默分叉的真相源」，必须收敛到一个** | `S-04:9`、`S-08:14`、`S-15:11` | **已落地（方向）** —— `D-016`（映射表 schema 单一权威）。**但「同一口径不得在两处独立维护」这条可机械检查的规则 `未落地`** → 应落到 `D-015` 的 AST 测试：同名口径常量只允许一个定义点。⇒ **已落地 2026-08-27**（登记册 **`L-39`**，wave 1–3 实现）：`download.py` 与 `locate.py` 各自的常量区，同名口径常量各只有一个定义点 |
| C-A3 | **审计字段豁免「零读者⇒可删」**：`durable audit timing is useful independently of a current reader` | `S-08:24`、`S-09:15` | **已落地（方向）** —— `D-003`。~~**但把它写成一条可执行的删除禁令 `未落地`**~~ → **已落地 2026-08-27**（登记册 **`L-41`**，`DECISIONS.md` **`D-003` 判据补充**）。**不是新决策**：`D-003` 原本就写了反转触发条件「某些字段从未被复核者使用 → 删掉它们」，缺的是「由谁、按什么程序删」。补的两条判据必须同时满足：① **单独立项**——删除动作必须走一次 OpenSpec 变更，提案里写明在哪一批复核数据上判为零使用、样本量多少，不许夹带；② **明确豁免「无消费者即死代码」**——`rules/commands.md` 同一天落的那条清理规则**明写不适用于证据链字段**，因为它的消费者是未来的复核者而不是当前的调用点。理由写成：字段删掉之后**此前所有回答的证据链都变得不可复核**，与 `D-012`「不修改后重用」同源。以下为原文，保留不改：应落到 `D-003` 增补：证据链字段的删除必须单独立项，不适用「无消费者即死代码」的清理规则 |
| C-A4 | **删除的边界是「所有能让它复活的入口」，不是那个类**；bug 侧实证：**堵路径不等于堵能力，serve 模式本身必须失败** | `S-11:11`、`B-15:31` | **`未落地`** → 应落到 `D-017` 增补：排除 `eval()` 不只是不调用它，而是**不留任何能到达求值的入口**（含三方库的 YAML `!!js` 方言，见 C-A5），并由 `D-015` 的 AST 测试强制。⇒ **已落地 2026-08-27**（登记册 **`L-33`**，wave 1–3 实现）：`formula` 自建封闭解析器不留任何求值入口；`mapping.load_pdf_mapping` 一律 `yaml.safe_load` |
...
| C-A6 | **不给默认，逼显式选择，配错则在加载时失败**：`sampleOverCapGlobResults` 必须显式配置，「没有任何产品级证据能确立其中一种顺序是隐含契约」 | `B-10:23` | **`未落地`** → 应落到 `D-018` 增补：口径类开关（口径版本、抽样策略、单位换算）**不设默认值**，缺失即在读入边界 fail-closed，而不是取一个「合理默认」。⇒ **已落地 2026-08-27**（登记册 **`L-34`**，wave 1–3 实现）：`locate.bind_columns` 抽不到单位 / 币种即 `SheetHeaderNotFound`；列角色解不出即 `None`，由调用方 fail-closed |
| C-A7 | **`?? {}` 把「没配置」变成「配了一个空对象」**，可选特性因此默认开启 | `S-21:13` | **`未落地`** → 应落到 `rules/failure-modes.md` 新增一条（与 F-2「只有真实字段能诚实触发才声明 flag」同族）：**空对象/空列表兜底会把「未声明」读成「已声明为空」**，配置读入处禁止 `?? {}` / `or []` **→ 已落地 2026-08-26（登记册 `L-21`，`rules/pitfalls.md` 第 14 条）。⚠️ 本行原写落地点为 `failure-modes.md`，那是**标错了**——那份文件只放「我们自己真犯过的错」，外部项目犯的错属预防性告诫，归 `pitfalls.md`。** |
| C-A8 | **「未知」是一等状态**：`TOOL_NOT_STARTED` / `TOOL_OUTCOME_UNKNOWN` / 完成，三态而不是两态；未知态**指示模型先核实外部状态或问用户** | `B-06:21` | **部分落地** —— `ARCHITECTURE §8.2`（`Refusal` 封闭枚举）+ `§8.3`（`completed_degraded`）已是三态骨架。**但「抽取尝试过但结果未知」与「未抽取」的区分 `未落地`** → 应落到 `§8.2` 增一个枚举值，并在 `EVAL_CASES §5.0` 与 `UNPARSEABLE` 并列。⇒ **已落地 2026-08-27**（登记册 **`L-10`**，wave 1–3 实现）：`record.RetrievalOutcome.ATTEMPTED_UNKNOWN`；`pipeline` 在区间内命中多行时走第三态，不在两个已知态里挑一个报 |
| C-A9 | **按声明的类型识别，不按形状猜**：`isCompactCheckpointSource` 是接缝自己的契约；按形状识别读的是「今天的生产者恰好长这样」这个巧合 | `B-17:19,33` | **`未落地`** → 应落到 `D-016`：抽取记录的类别由**显式 `kind` 字段**判定，禁止用「看起来像三大表某一行」这类形状启发式反推。⇒ **已落地 2026-08-27**（登记册 **`L-35`**，wave 1–3 实现）：`mapping` 的 `kind` 只接受枚举成员名，无任何由文本或形状反推类别的入口（有负向用例） |
| C-A10 | **命名不得夸大持久事实**：`Naming it delivery would overstate the durable fact`；分派 ≠ 投递，可见 ≠ 被读 | `S-35:29` | **已落地（方向）** —— `D-003` + `ARCHITECTURE §8.3`（`completed_degraded` 就是拒绝夸大）。~~**但「状态名必须是它能证明的那件事」这条命名规范 `未落地`** → 应落到 `§8.2` 的枚举命名约束~~ → **已落地 2026-08-26**（登记册 **`L-11`**，`ARCHITECTURE` **§8.2.1**）。落地时加了两样原文没有的：① 可机械化的一半——**每个枚举值旁必须写一句「它凭什么证据成立」**，写不出来就改名或删掉；② 抽取侧的三个高危词清单（`extracted` / `verified` / `matched`），每个都注明「它实际只能证明什么」 |
| C-A11 | **权威源逐条指名，自算公式只能是估算**：`A hard-coded provider-neutral estimate would look authoritative while being wrong` | `S-22:27`、`S-24:19` | **已落地（方向）** —— `D-013`（数值以年报 PDF 原文为准）。**但「估算值与记账值必须分字段、估算值不得进入证据链」`未落地`** → 应落到 `AC-05`（字段有真值）相邻：派生指标必须标 `derived` 并携带其输入字段的指针。⇒ **已落地 2026-08-27**（登记册 **`L-54`**，wave 1–3 实现）：`formula.field_refs()` 按出现顺序列出输入指针 + `ComputeResult.inputs` / `derived` |
| C-A12 | **把「以后新增的也必须遵守」写成自动发现 + 拒绝的测试**，而不是 README 里的约定 | `B-18:13`、`B-13:23`、`S-27:17` | **已落地（方向）** —— `D-015` 的 AST 测试就是这个形态。**但「新增数据源/新增口径必须被测试自动发现」`未落地`** → 应落到 `D-015` 扩一条：遍历数据源注册表，拒绝任何未声明口径版本的条目。⇒ **已落地 2026-08-27**（登记册 **`L-40`**，wave 1–3 实现）：`mapping.load_pdf_mapping` 对缺失 / 非法 `mapping_version` 直接 `raise ValueError`（有负向用例） |
...
| C-A15 | **验证之前先确认你测的是刚构建出来的产物**；`CI 从未暴露，只有本地脚本暴露` | `B-13:82` | ~~**`未落地`**~~ → **已落地 2026-08-27**（登记册 **`L-22`**，`rules/commands.md` 新增「第 0 步：确认被测对象是当前工作树的产出」一节）。**只落了 `commands.md` 那一半**：`failure-modes.md` 那一半按该文件 2026-08-27 新增的准入条件筛掉了（外部项目的错不是我们的事故）。⚠️ **但落地当场在本仓复现了一次**：独立 worktree 里跑主仓 `.venv` 的解释器，`python -m semantic_layer scan` / `validate` 因 editable 安装的 `.pth` 钉死绝对路径而**读的是主工作树的代码**，`pytest` 却因 `pythonpath = ["src"]` 走 rootdir 而读 worktree——**同一解释器、同一 cwd，导入两份代码，输出里没有任何东西会说明这件事**。该实例已连同 `PYTHONPATH` 的修法写进那一节。以下为原文，保留不改：应落到 `rules/commands.md`：冻结校验与门禁命令前置一步「确认被测对象是当前工作树产出」，并在 `rules/failure-modes.md` 记为「陈旧产物导致的假通过」 |
| C-A16 | **从现状派生的覆盖检查，永远查不出「被遗漏的那一类」** | `B-13:27` | ~~**`未落地`**~~ → **已落地 2026-08-27**（登记册 **`L-23`**，`rules/pitfalls.md` **第 19 条**）。⚠️ **落地点改了**：原写 `rules/failure-modes.md`，但那份文件 2026-08-27 新增的三条准入条件把它筛出去了——**规则是从外部项目读出来的预防性告诫，不是我们犯错后写下的事故记录**。落地时补了两样原文没有的：① 一句可当场执行的判据——问「这个检查的『应该有什么』是从哪来的」，答案若是「从已经处理过的那些里数出来的」，它在构造上只能查损坏、查不了遗漏；② **本仓的一个真实实例**——第六道门的 **R5** 的基准集合（哪些算已落地）取自登记册自身，于是本轮查出的三条「内容已落地、登记册仍标 `OPEN`」（`L-74` / `L-82` / `L-83`）**R5 一条都发现不了**。该实例三条准入条件都填得满，**是否据此在 `failure-modes.md` 另立一条事故留给操作者**。以下为原文，保留不改：应落到 `rules/failure-modes.md` 新增一条：覆盖率/完整性检查的**基准集合必须独立于被检查物**（不得从「已经处理过的字段」反推「应处理的字段」）。与 F-3「如实标注读得少 ≠ 读过」同族但更硬 |
| C-A17 | **不确定时不复用**：未匹配、冲突映射、长度变了的数组、同时含语义与易变变化的字符串 —— 一律用新值 | `B-11:19,33` | **已落地（方向）** —— `D-018`（fail-closed 在读入边界）。**「证据对齐失败时不得部分复用旧指针」这条 `未落地`** → 应落到 `ARCHITECTURE §8.4`。⇒ **部分落地 2026-08-28**（登记册 **`L-12`**，wave 4 Task 3）：`extractor/crosscheck.align_references()` 把这三种情形逐条实现在**对照参照集**上 —— 字段集变 / 同一 `field_id` 的列名变 / `mapping_version` 变，任何一条成立即整体返回新值，四条用例锁住。**整条证据链的指针对齐仍未落**，被 `U-01` 正当挡着（证据链字段集要等 Phase 2 的复核实验数据），**不得把这半个读成整条** |
| C-A18 | **保证的措辞削到实现真能满足的强度**：「至多一个 turn」而不是「恰好一个」；`Harness 不声称通用的 exactly-once 效果`；`缓存复用是尽力而为，正确性不是` | `S-10:17`、`B-06:21`、`B-05:23` | ~~**`未落地`**~~ → **已落地 2026-08-27**（登记册 **`L-53`**，`PROJECT_SPEC` **§9.1**）。十条逐条分了类：**保证 8 条**（AC-01 / 02 / 03 / 04 / 05 / 08 / 09 / 10，其中 AC-05 与 AC-09 另标了「覆盖面窄于字面」与「措辞偏弱」）、**尽力而为 2 条**（AC-06 / AC-07 —— 它们是对一次测量的阈值，不达标时没有任何东西会挡住任何事）。落地时补了原文没有的两样：① **每条的可观测后果**具体到哪条命令会非零退出；② **「尽力而为」限定的是结果、不是动作**——实验必须跑、数字必须写出来。⚠️ **没有改动任何一条 `AC-xx` 的措辞**：AC-05 的覆盖面与 AC-09 的强度确实该改，但那是规格变更，**已标出、留给操作者**。以下为原文，保留不改：应落到 `PROJECT_SPEC` 的验收标准措辞审查：每条 `AC-xx` 标注它是**保证**还是**尽力而为**，并写清失败时的可观测后果 |
| C-A19 | **「重新引入的门槛」作为删除记录的固定一节**：要回来必须带一个具名消费者 + 它自己的失败/生命周期/重放/测试契约 | S-11/S-22/S-23/S-25/S-28/S-29/S-33/S-37/S-39/S-43/S-44 共 11 篇 | ~~**`未落地`**~~ → **已落地 2026-08-27**（登记册 **`L-42`**，`DECISIONS.md` **`D-009` 判据补充**，与 C-N7 合并落成两个必填节）。落成一张两行的表：**「放弃了什么能力」**（不许写概括，要写具体拿掉了哪个能力）与**「重新引入条件」**（需要出现哪个具名消费者、它必须自带什么契约）。**两节都不许写「暂无」**：写不出前者说明删的东西本无能力可言，写不出后者说明删除理由没想透。⚠️ **模板文件本身没改**——`openspec/` 的模板由工具链拥有，改它属工具配置不属决策，须操作者裁决；本条只定内容要求。以下为原文，保留不改：应落到 `D-009`（三方法论减法触发）：OpenSpec 归档模板增加「重新引入条件」必填节 |
| C-A20 | **「有报错」不等于「报对了」**：装配后确实 fail loud，但错误点名的是消费者不是配错的提供者；因此加**加载期守卫**直接报出责任方 | `S-41:13,22` | **`未落地`** → 应落到 `ARCHITECTURE §8.3`（拒绝留痕）：`Refusal` 必须携带**责任方标识**（哪个数据源 / 哪条口径规则拒绝的），不能只说「证据不足」。⇒ **已落地 2026-08-27**（登记册 **`L-9`**，wave 1–3 实现）：`extractor/download.py` 的 `SOURCE_*` 常量 + `formula` / `pipeline` / `reconcile` 各自的 `source=`；每个 `Refusal` 构造点都带非空责任方 |
...
| C-N2 | **「静默但写进文档」作为可接受形态**：启动器不校验产物新鲜度，陈旧包可静默提供旧浏览器代码，配套做法是写进 onboarding 与 CLI 参考 | `S-45:17,32` | 对开发者工具，「文档 + 显式两条命令」是合理取舍。**对证据链不成立**：读者不读我们的 README，他读的是那条答案。陈旧口径必须在**读入边界**被拒，而不是在文档里被预告 | **已落地** —— `D-018` + `D-012`。**但「文档不能替代闸门」这条明文 `未落地`** → 应落到 `rules/pitfalls.md` 新增一条（接在第 11 条「不可逆压缩」之后） **→ 已落地 2026-08-26（登记册 `L-29`，`rules/pitfalls.md` 第 13 条）。** |
| C-N3 | **不闩住第一个后台错误**：保留失败批次但允许重试，「可观测性与重试两者都保住」 | `S-15:27-33` | 他们的取舍在「确定性」与「可恢复性」之间，选了后者，因为丢的是一次持久化写。本项目的**闸门是批次级**（`D-018`），一批里出现证据写入失败时**必须闩住这一批**，否则重试成功的部分与失败的部分会混进同一次回答 | **已落地** —— `D-018`（闸门批次级）。**「重试不得跨闸门批次」这条 `未落地`** → 应落到 `ARCHITECTURE §8.4`。⇒ **已落地 2026-08-27**（登记册 **`L-13`**，wave 1–3 实现）：`record.ExtractionBatch.seal()`，闩死后 `add_record` 一律拒绝；`reconcile` 判定完即闩 |
| C-N4 | **脱敏/口径在消费时求值，而不是在追加时固定**：「脱敏在 feedback 时求值……feedback 之前的一次脱敏策略变更会影响那次重放」 | `S-30:17,29` | 他们诚实标注了这个语义差异，对遥测可接受。**对可复核证据不可接受**：同一条证据在两个时间点复核会得到不同结果，`D-012` 的冻结就失去意义 | **`未落地`** → 应落到 `D-012` 增补：口径版本号必须**随抽取记录一起冻结**，复核时按记录里的版本求值，而不是按当前配置。⇒ **已落地 2026-08-27**（登记册 **`L-38`**，wave 1–3 实现）：`mapping.PdfMappingTable.mapping_version` 随 `ExtractionRecord.mapping_version` 一起冻结 |
| C-N5 | **匹配不到任何行的 patch 保持 no-op 而不是报错**（他们明写这是有意的，因为个人 overlay 要跨多个面共享） | `S-21:43` | 前提是「多面共享」。本项目的映射表是**单一权威**（`D-016`），一条匹配不到任何字段的映射规则只可能是写错了。同样是 no-op，他们那里是设计，我们这里是缺陷 | **`未落地`** → 应落到 `D-016`：映射表加载时**零命中的规则即加载失败**（同 C-A6 的「配错则在加载时失败」）。⇒ **已落地 2026-08-27**（登记册 **`L-36`**，wave 1–3 实现）：`pipeline.extract_records` 整份 PDF 零命中即 `MappingZeroHit`；与 `ROW_ABSENT` 显式分开 |
| C-N6 | **靠匹配上游错误措辞做分类**（`terminated` / `stream ended before message_stop` → `TRANSPORT`） | `B-07:15,35` | 他们自己写死了残留风险：`a future pi-ai release that rewords these errors would silently fall back to PI_AI_ERROR`，并留了 `XXX` 与 Known-Limitations。**这个处置方式（承认 + 留锚点 + 写限制）该学**，但**分类本身不能用在证据链上**：`UNPARSEABLE` 之类的归因若依赖 PDF 文本措辞，上游排版一变就静默退回「正常」 | **部分落地** —— `EVAL_CASES §5.0` 的 `UNPARSEABLE` 已存在。~~**「归因分类不得依赖上游可变措辞」这条判据 `未落地`**~~ → **已落地 2026-08-27**（登记册 **`L-50`**，`EVAL_CASES` **§5.3**）。落地时补了原文没有的两样：① **为什么本项目比 harness 更需要这条**——`D-013` 把数值权威源定为年报 PDF 原文，而 PDF 排版每家每年都不一样且改版不通知任何人；② **三种可用的判据形态**（结构位置 / 口径标识 / 闸门返回值，都与 PDF 文本无关），以及「确实只能靠文本判的必须显式登记进已知限制」。**当前已知限制记 0 条**，理由写明：抽取器尚未产出真实归因，现在写下的任何一条都会是 F-2 那种求不出值的规则。以下为原文，保留不改：应落到 `EVAL_CASES §5` |
| C-N7 | **被取消的提案直接删除，而不是保留成 active 或 rejected 记录**（只把动机、放弃的能力、重新考虑的条件合并进本篇） | `S-44:21` | 他们的做法在「笔记体系自洽」这个前提下成立（`S-15:7` 的自我标注、`S-37:21` 的动机继承都是配套机制）。本项目 `openspec/changes/` 的归档记录**本身就是产物**——决策轨迹要能被外部复核，删提案会让「为什么当初没这么做」变成不可复核的 | **已落地（方向）** —— `D-009`（OpenSpec 走 archive 而非删除）。~~**但「归档提案必须写清放弃了什么能力」这条 `未落地`**~~ → **已落地 2026-08-27**（登记册 **`L-42`**，与 C-A19 合并落成 `D-009` 判据补充的两个必填节）。落地时按 `references/README.md` 第 4 条**写清了不借的那一半**：借它的**内容清单**（动机 / 放弃的能力 / 重新考虑的条件），**不借它的删除动作**——他们的做法在「笔记体系自洽」这个前提下成立，而本项目 `openspec/changes/` 的归档记录本身就是产物，删提案会让「为什么当初没这么做」变成不可复核的。以下为原文，保留不改：应落到 `D-009` 的归档模板（与 C-A19 合并做） |
