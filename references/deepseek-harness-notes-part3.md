...
| # | 借鉴什么 | 出处 | 落地点 |
|---|---|---|---|
| C-A1 | **抽样/排序/截断的口径必须随结果一起呈现**：超上限时页脚明说「这是跨条目抽样而不是修改时间头部」，并报告触及了多少个顶层条目；只要 spill 成功，**完整排序列表保留在 artifact 里** | `B-10:15-17` | **`未落地`** → 应落到 `ARCHITECTURE §8.5`：证据指针必须携带**取数口径三元组**（抽样方式 / 排序键 / 截断位置）且被下游强制消费；并在 `EVAL_CASES §3.3` 增一条判据：答案引用的表格片段若来自截断页而未声明口径，记失败 |
...
| C-A14 | **负控制**：把守卫删掉、重新构建、重跑，断言**必须变红**；否则你测的是一个空洞的东西 | `B-13:76` | **`未落地`** → 应落到 `EVAL_CASES §3.3` 增设 **J-5**：闸门类断言（fail-closed、拒绝留痕）必须配一条负控制用例，证明去掉闸门后该用例会失败 |
