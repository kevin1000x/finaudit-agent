...
| 2 | **映射表的 `label_fragments` 是茅台专属的** —— 万华把总权益行写成 `所有者权益（或股东权益）`，**没有「合计」二字**；茅台是三行缝合且以 `合计` 结尾 ⇒ `L-36` 零命中，加载即失败 | ⬜ **未做**：要为每个字段补第二家公司的 `label_variants`，25 个字段的数据工作 |
...
**第 1 层已经不再是障碍**，这是 `SC-1b` 第一次在**第二家公司**上成立。
⚠️ 但 **`SC-2`（取到的值正确）在万华上仍是零证据** —— 卡在第 2 层。
