...
### C.1.2 ⬜ 那条「显然该加」的缝合规则量过了，是错的（**2026-09-02 裁决不做，见 `N-49`**）

虽然第 2 层已经不需要它，这次测量仍要留档 —— **它挡住了一个会弄坏一切的改动**。

➜ **2026-09-02 操作者裁决：这件事不做**，已移入台账 **X 区（无承诺）** 的 `N-49`，
带重新考虑的触发条件（出现某字段的数值不印在标签行上、必须靠缝合才取得到）。
⚠️ **不是「缝合器不需要改」，是「今天没有证据要求改」** —— 没有证据不是反面证据。
...
➜ **2026-09-02 补齐并由操作者签署**：25 个取值已逐字段核对，**25 / 25 一致，零不一致**（§D.18）。
...
**C.9 `N-42` 未做的一半 —— 已做，且又找出一处。**

⚠️ **这一条此前一直写着「本轮没查，记 `UNVERIFIED`」，而台账 `N-42` 里 08-31 就已经逐道核完了。**
正文更新而这里没跟上，**又一个 `N-35`**。以下是当前事实：

- **2026-08-31**：其余六道门逐道核对完毕（表见台账 `N-42`）。结论**不是「都有」也不是「都没有」**：
  `check_reading_ledger.py` 与 `semantic_layer scan` 的内容扫描**有同一缺口，已修**；
  `forbidden_tracked_file_types` 用「已跟踪」**是对的，不许跟着改**（未跟踪的 PDF 是设计的一部分）。
  同轮炸出更深的一处：三处 `git ls-files` **都没有 `-z`**，非 ASCII 路径会被静默跳过 —— 已全部改。
- **2026-09-02 又找出一处，形态相同**：`check_gates.py` 只扫 `TESTS_DIR.rglob("test_*.py")`，
  ...
  **当场量的，不是推演**（临时放一份 `tests/zz_probe_test.py`，一个函数、零断言）：

  | 文件名 | pytest | `check_gates.py` |
  |---|---|---|
  | `zz_probe_test.py` | `1 test collected` | 扫描 **34** 个模块 ⇒ **通过**（看不见它） |
  | `test_zz_probe.py` | 同上 | 扫描 **35** 个模块 ⇒ **不通过**（R1 抓住） |

  本仓当前 `*_test.py` 有 **0 个文件**，所以它一直表现为一切正常。
  ✅ **已修**：扫描范围改为按 `PYTEST_FILE_PATTERNS` 两条走，并新增三条测试 ——
  一条锁「常量与 pyproject 两个声明对得上」，一条锁「扫描真的按两条走」，一条锁去重。
  **两条变异各自验过**：只改常量 ⇒ 2 条红；常量不动只窄化扫描 ⇒ 1 条红（证明两条不冗余）；
  回退后 82 passed，`check_gates.py` 的 SHA-256 与备份一致。

⬜ **仍未做的**：`check_gates.py` 里另外两处**文件系统 glob** 的范围（`references/*.md` 非递归、
`metrics/*.yaml` 非递归）今天**量过当前值**（27/27、21/21，无子目录）**但没有加锁** ——
...
**C.11 `D-030` 的余量在收窄，且趋势向下 —— 但最近一次没读到数。**

akshare 近月下载量历次实测（门槛 3,000,000）：
...
🔴 **2026-09-02 那次供应链门禁 exit 0，但 4 项下载量检查全部因 `HTTPError` 跳过** ——
`D-030` 判据 2 本次**没有生效**。⇒ **「门禁通过」在这里不等于「余量仍在门槛之上」**，
这两句话必须分开说。当前最新有效读数仍是 08-31 的 3,482,602。

斜率约 −50,000/天（08-29 → 08-31）。**跌破即红，处置不是再下调一次** ——
再下调就把这道门变成跟着实测值走的橡皮门槛。届时的选项是接受它红并停用
`crosscheck` extra，或另找对照源。

⚠️ **「取不到读数」这个状态本身也要盯**：它连续出现时，这道门实际上是**哑的**，
而哑与绿在退出码上不可区分 —— 与 `N-42` 同族。台账 `N-44` 记着这件事。
...
- ✅ **2026-09-02 操作者签署：接受本节的 25 / 25。**
  ⚠️ **签的是「这份 legwork 成立」，不是「`SC-2` 在万华上成立」** —— 见下一条。
- ⚠️ **效力限定与 `SC-2` / `SC-8` 完全相同**（§C.13）：读数路径独立于抽取器，**执行者不独立**。
  **这条限定不随签署消失** —— 它是这份证据**是什么**，不是它**够不够**。
- ⚠️ **签署也不消除 `N-48`**：那 2 条免检字段**仍然是免检的**。
...
#### D.19 2026-09-06 万华 3 个 `notes.*` 补核：产品要能答万华，缺的就是这三个

**起因不是「顺手多核几个」。** `N-61` 的离线导出把「只导出人工核对过的字段」
做成了硬约束，于是万华只导出 §D.18 核过的 25 个 —— 三个 `notes.*` 被挡在外面。
实跑「600309 2019 年的资产负债率」当场拿到：

```
UNEVALUABLE_CONDITION：标记 'restated' 的触发条件求值失败：
'notes.restatement_flag == true'：比较运算的操作数 notes.restatement_flag 缺失。
```

⇒ 少这**一个**字段，万华**一道题都答不了**。可比性标记求不了值就是拒答（`D-022`：不猜）。

**方法与 §D.17 / §D.18 一致，故意不换**：读数走
`pdfplumber.open(...).pages[N-1].extract_text(layout=True)` 的原始版面文本，
**不经过 `locate` / `mapping` 任何一层**。样本 `data/raw/600309_2019.pdf`（213 页）。

| # | 字段 | 页 | 版面原文（逐字） | PDF 上的值 | 抽取器 | 判定 |
|---|---|---:|---|---|---|---|
| 1 | `notes.business_combination_type` | 169 / 171 / 173 | `1. 非同一控制下企业合并` \| `√适用 □不适用`；`2. 同一控制下企业合并` \| `√适用 □不适用`；`3. 反向购买` \| `□适用 √不适用` | {同一控制, 非同一控制} | 同值 | **一致** |
| 2 | `notes.consolidation_scope_change` | 173 | `5. 其他原因的合并范围变动` \| `√适用 □不适用` \| 表内 `万华化学(四川)有限公司 · 化工产品生产及销售 · 新设` | `True` | 同值 | **一致** |
| 3 | `notes.restatement_flag` | 6 | 表头三处印 `调整后` / `调整前`，且两列**数值不同** | `True` | 同值 | **一致** |

**3 / 3 一致，零不一致。**

##### 哪一条是真正的辨析测试

🔴 **#1 是。** 合并范围那一节下面挂着**三个**子项，勾选状态是 `√ / √ / □`。
一个「有就全返回」的实现会多返回 `REVERSE_ACQUISITION`，
一个「有就返回第一个」的实现会漏掉同一控制那一支 —— **两种错都会在这里露馅**。
取到的恰好是勾了的那两个，第三个正确地没进来。

**#2 有自己的版面依据，不是从 #1 推出来的。** p173 子项 5 单独勾了 `√适用`，
表内列着新设的万华化学(四川)。⇒ 「合并范围变了」这句话有独立出处。
（`D-020` 把这两个字段拆开的理由，在万华这份年报上再次被证实：
茅台是 `∅ + True`，万华是 `{两支} + True` —— 两个字段在同一份年报上取值不同。）

🔴 **#3 比茅台那次强一档。** §D.17 #8 在茅台上的判据只有**列头存在性**
（印了 `调整后`/`调整前`）。万华这里两列的**数值本身不同**：
2018 年营业收入 `72,837,108,238.47`（调整后）vs `60,621,193,436.91`（调整前），
差 12,215,914,801.56。⇒ 重述**真的发生了**，不是只印了一个表头。

**而且它与利润表那一路互证**：`is.operating_revenue_prior_as_presented`
取的是 p75 利润表 `2018年度` 列的 `72,837,108,238.47`，
与 p6「近三年主要会计数据」表 `2018年 调整后` 列**逐字相同**。
⇒ 年报里两个互相独立的位置给出同一个数，这比「我读到了这个数」强一档。

##### 状态与限定

- ✅ **2026-09-07 操作者签署：接受本节的 3 / 3。**
  ~~本节是执行者产出的 legwork，尚未由操作者签署。~~
  签的是「这份 legwork 成立」——`data/verified/600309_2019.yaml` 里
  `signed_off`（§D.18，前 25 个）与 `signed_off_d19`（本节，末尾 3 个）**分开记**，
  因为它们是两份不同的证据，不是一份证据的两半。
- 🔴 **这三条已经进了导出**，理由写在这里而不是藏着：拦住它们的代价是**整家公司不可用**，
  而它们已经过了与前 25 条同一套方法的核对。**这是一次权衡，不是一次遗漏。**
- ⚠️ 效力限定与 `SC-2` / `SC-8` 相同：读数路径独立于抽取器，**执行者不独立**。
- ⚠️ **这不改变 `SC-2` 的任何计数。** `SC-2` 的分母是茅台那 29 个，与本节无关。
...
| 验收标准 | 验证方法 | 实际命令 | 期望结果 | 实际结果 | 证据 | 状态 |
|---|---|---|---|---|---|---|
| AC-05 证据链完整率 | 批量评测 | `python -m eval.run --suite frozen-01` | 100% | **15/15**（其中 8 题按收窄键集计） | §F.1 | ✅ |
| **AC-06 复核一致率**（H2 门） | 复核实验，限时 5 分钟 | `python -m eval.h2 --sheet …` | ≥80% | `h2-01` **13.3%（2/15）**；`h2-frozen-02` **100%（21/21）但不作数**（`W=0` 且题包非盲出，器械机械挡下并退非零） | §F / §G | 🔴 `UNVERIFIED` |
| 「无法判断」占比 | 同上 | 同上 | ≤20% | `h2-01` **86.7%（13/15）**超限 4 倍；`h2-frozen-02` **0%（0/21）** ✅ | §F / §G | ✅ **达标**（§4.3 那道强制重测已完成） |
| AC-08 归因覆盖率 | 批量评测 | `python -m eval.run --suite frozen-01` | 100% | **4/4** | §F.1 | ✅ |
| AC-09 危险样本拒绝 | 静态校验测试集 + 负控制 | `pytest tests/test_gate.py` | 全部拒绝且负控制成对 | 5 个样本各一对 | `tests/test_gate.py` | ✅ |

### §F — H2 复核实验第一轮（`h2-01`，2026-09-06，**判定：不成立**）

**这一轮是普查不是抽样**（`D-037` / `EVAL_CASES` §4.1.1）：可计分池 `N = 15`，
其中答错 `W = 4` ⇒ **`≥5 题答错` 未满足**；错答案子集上一致率的最小刻度为 `1/4 = 25 个百分点`。

| | |
|---|---|
| 复核者 | 操作者本人（有财务背景）。⚠️ **不独立于本项目**，见 `02-03-PLAN` 限定 3 |
| 一致率 | **2/15 = 13.3%** —— `EVAL_CASES` §4.2：**< 60% ⇒ 停止，回到 ARCHITECT 重设计证据链，不得进入 Phase 3** |
| 无法判断 | **13/15 = 86.7%** —— §4.3 上限 20%，**超限 4 倍** |
| 判「对」的题数 | **0** |
| 错题子集 | **2/4**（原始计数，§4.2 附注强制） |

#### 🔴 结论要这么读，不能那么读

**不能读成**「系统答案大量错误」。13 道「无法判断」按协议不计入一致，
一致率是被它们**机械地**拉下来的。复核者的原话：
「最直接的结论不是『系统答案大量错误』，而是**当前人读证据页没有暴露足够的底层证据来支持独立复核**」。

**两个方向都要记住**：

- 🔴 **坏消息**：`D-003` 说证据链是第一类产物、给不出证据链即失败。
  这一轮量出来的是：**证据链在 13/15 的题上不足以支撑独立复核**。这是 `D-003` 的核心主张第一次被实测否掉。
- ✅ **但失败的形状比预想的好**：`02-03-PLAN` 的 `<context>` 当时担心的是
  「证据链把一个错误的结果讲得足够有道理，以致复核者认可了它」——
  **这件事一次都没发生**：判「对」的题数是 **0**，复核者没有背书任何一个错误结果。
  失败模式是**「说不出对错」**，不是**「被骗」**。前者靠补证据可修，后者要重设计。

#### 三个证据缺口（复核者逐题给出的理由，归成三类）

| 缺口 | 题号 | 缺的是什么 |
|---|---|---|
| **G1 算出来的数没有输入** | H2-02 / 07 / 09 / 13 / 14（**全部 5 道计算题**） | 有定义、有公式、有结果，**没有实际参与计算的原始字段值**。原话：「执行哈希能标识一次执行，不能代替计算输入」 |
| **G2 拒答条件触发了，但看不见触发它的那个值** | H2-06 / 12 | 系统称「比较期金额缺失」「营业收入 ≤ 0」，页面**没展示那个字段实际是 `null` / `0` / 负数** ⇒ 只知道它触发了，不知道**触发得对** |
| **G3 说「没这个指标」，但拿不出当时的指标清单** | H2-01 / 04 / 08 / 10 / 11 / 15（**6 道**） | 没有当时语义层的指标·别名清单或可验证快照 ⇒ 无法独立证明 `METRIC_NOT_DEFINED` 是对的 |

⚠️ **G1 的数据其实已经算出来了，只是没显示**：`src/agent/answer.py` 把
`inputs`（公式引用的每个字段的取值）喂进了执行哈希，**从未渲染到页面上**。
「算进哈希」与「给人看」是两件事，这一轮把差别量出来了。

#### 复核者顺带查出的两个真缺陷（**不是证据问题，是系统问题**）

这两道是仅有的两个「错」，且与本次运行的 `FAIL` 判定一致 ——
**但复核者给的理由比系统自己的归因更准**：

1. **H2-03（`Q-C3-004`）**：系统以 `INTENT_INCOMPLETE`「题面里出现多个期间…取第一个是猜」拒答。
   复核者：**题面明确要求比较 2018 与 2023，两个期间正是提问意图**，不构成「意图不完整」。
   ⇒ 拒答理由本身是错的，不只是结果错。
2. **H2-05（`Q-C3-003`）**：系统称「题面里没有主体（六位股票代码）」。
   复核者：**题面已给出「华鑫科技」。「没有六位代码」≠「没有主体」，公司名称本身就是实体信息。**
   ⇒ `src/agent/intent.py` 把「主体」等同于「六位代码」，这是一个实打实的定义错误。

#### 缺口的处置与验收（2026-09-06 补记）

三个缺口由 `D-038` 补齐，**复核者看过改后样张，回复「三节够了」** ⇒ 修补被验收。

⚠️ **验收的是「缺口补上了」，不是「一致率变好了」。** 两件事：

| | 状态 | 凭据 |
|---|---|---|
| 缺口补上了 | ✅ | `tests/test_answer.py` 三组回归（`test_G1_*` / `test_G2_*` / `test_G3_*`），各带造回归看红 |
| 补完之后一致率是多少 | ⬜ **没有读数** | —— |

⇒ **`AC-06` 仍是 `UNVERIFIED`**。这不是「还欠一项工作」，
这是**我们手上没有改后的读数**这个事实。**不为重测单独立项**
（操作者 2026-09-06：不要为此增加重复工作量）——
什么时候测跟着 `N-46`（建 `frozen-02`）或「有第二位复核者」走。

#### 这一轮**不可**用同一份题包重测

复核者现已看过全部 15 题与自己的判定。同人同题重测**测的是记忆，不是证据链**。
⇒ `h2-02` 需要新材料（连带 `N-46` / `frozen-02`），或换一位复核者。**两条都要操作者定。**

### §G — `frozen-02` 那一轮（`h2-frozen-02`，2026-09-10）—— **不构成 §4.2 的通过读数**

新材料这条路走了：`frozen-02` 于 `2026-09-09T14:13:01Z` @ `d7e9295` 冻结，
21 题普查，复核者仍是操作者本人（有财务背景，不独立于本项目）。

```text
$ .venv/Scripts/python -m eval.h2 --suite frozen-02 --sheet docs/agent/h2-frozen-02/answer-sheet.yaml
  一致率        21/21（100.0%）
  无法判断      0/21（0.0%，上限 20%，未超限）
  错题子集      0/0（原始计数，§4.2 附注强制）
🔴 **这一次不构成 §4.2 的通过读数**
  判定：**不给「通过」。**
退出码 1
```
...
| 读数 | 成不成立 | 为什么 |
|---|---|---|
| §4.3 反向检验（「无法判断」≤ 20%） | ✅ **0/21 = 0%** | 不依赖错题子集。`h2-01` 在这里是 86.7%（超限 4 倍），§4.3 因此**强制**重测 —— **这一项现在达标了** |
| §4.2 一致率（≥ 80%） | ❌ **不作数** | 两个前提都不成立，见下 |

🔴 **一致率 100% 为什么不能记成 `AC-06` 通过**，两条各自独立成立：

1. **`W = 0`**：21 题系统全对，一致率整个落在**正确答案**上。
   §4.2 附注说的分辨信号来自**错题子集**，而这里错题子集是 `0/0` ——
   测不到「证据页帮不帮得上抓错」。
2. **题包非盲出**（`N-75`）：执行者在交题包之前跑了评测、读了结果、
   并把「21/21 全对」告诉了复核者本人。复核者已知答案全对时，每个「对」都不含信息。
   `docs/agent/h2-frozen-02/BUILD.json` 记着 `"blind": false`。

⚠️ 这两条现在由 `eval/h2.py` **机械挡住**，不靠人记得写：
`score()` 的 `pass_blocked` 让 ≥80% 那一档在前提不成立时**不给「通过」**，
CLI 退非零。此前它只检查强制标注的**键在不在**、不看**值**，
于是同一屏上印出过「判定：H2 通过」与「blind_build: 未声明」两句自相矛盾的话
（2026-09-10 实测抓到并修，4 道回归各带造回归看红）。

#### 但这一轮**确实**产出了一件东西：证据页可重算的直接证据

复核者 21 条判词里逐条引了**参与计算的原始字段取值**，而那些值只有
`D-038` 的 `G1`/`G2`/`G3` 与新增的 `G4` 那几节提供。最硬的一条是 `H2-18`：

> 判词：「六项有息负债合计 **3000000000**，除以 12500000000 = 0.24」

**`3000000000` 这个数在整份 `packets.md` 里出现 0 次**（`grep -c` 实测）——
页面只给了六个分量各自的取值，合计是复核者自己加出来的。
⇒ 这一条**不可能**靠「已知答案全对」蒙出来，它只能来自页面。

其余可直接指认的：`H2-05` 引「可识别公司简称清单中没有『瑞捷』」⇒ 用的是 `G4`；
`H2-11` 引「**上期**比较营业收入缺失」⇒ 用的是修掉截断之后才分得开的那两行；
五道 `METRIC_NOT_DEFINED` 全部引「页面列出的全部可用指标名中没有 X」⇒ 用的是 `G3`。

⇒ **可以说的**：`G1`–`G4` 四节在真实复核中被实际使用了，页面**够重算**。
⇒ **不可以说的**：`AC-06` 通过。那需要 `W ≥ 5` 且题包盲出，两条都不满足。

**`AC-06` 仍是 `UNVERIFIED`** —— 但要读准这个标签的分量：

`PROJECT_SPEC.md:409` 把 `AC-06` 归为**「尽力而为」**类，同节逐字写着
「**尽力而为类不达标 ≠ 阻塞**」「我们保证的是**做这个实验并如实报数**，不保证达到 80%」，
以及「**尽力而为限定的是结果，不是动作** —— 实验必须跑、数字必须写出来」。

⇒ 到这里，规格要求的那个**动作**已经履行两次并如实报数（§F 与本节）：

| 规格要求的 | 做到没有 |
|---|---|
| 跑这个实验 | ✅ 两轮：`h2-01`（15 题）、`h2-frozen-02`（21 题，新冻结材料） |
| 如实报数 | ✅ 两轮的数都写出来了，含**不利于自己**的那些（13.3% / 86.7%；以及本轮那个 100% 为什么不作数） |
| 达到 80% | ⬜ **从来不是承诺**（本条是尽力而为类） |
| §10 停止条件（< 60%） | ✅ **未触发**。`h2-01` 曾触发，`D-038` 处置后本轮 `无法判断` 由 86.7% 降到 0% |

🔴 **`AC-06` 达不到 80% 不阻塞 Phase 3。** 阻塞的是一致率低于 60%，而那条现在不成立。
本文件此前有一处写成「`AC-06` 挡着 Phase 3」（`h2-frozen-02/README.md`），已更正。

**要把这个 `UNVERIFIED` 变成 `PASS`，需要同时满足 `W ≥ 5` 与题包盲出**（`N-74` / `N-75`）。
其中 `W ≥ 5` **不是能安排的工作**：§4.1.1 明禁为了凑够它去挑题、换套件或下调阈值，
所以它只能等系统在诚实构造的新题上真的失败。⇒ 不为它单独排期。
