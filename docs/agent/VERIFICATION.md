...
| **AC-06 复核一致率**（H2 门） | 复核实验，15 题限时 5 分钟 | `python -m eval.h2 --sheet …` | ≥80% | **第一轮 13.3%（2/15）**；缺口已补并经复核者验收样张，**改后无读数** | §F | 🔴 `UNVERIFIED`（第一轮不成立） |
| 「无法判断」占比 | 同上 | 同上 | ≤20% | **第一轮 86.7%（13/15）**；改后无读数 | §F | 🔴 `UNVERIFIED`（第一轮超限 4 倍） |
