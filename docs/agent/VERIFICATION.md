...
| 2 | **映射表的 `label_fragments` 是茅台专属的** | 🟡 **大部分已解决 —— 但先纠正一句我自己的错判** |
| 3 | **数值列不止两列** —— 万华那一行是 `43,931,258,971.63 / 39,582,324,008.33 / 27,853,995,769.94` **三列**（与它 `restated = True` 一致，列出了调整后/调整前）；现有列绑定按「期末 / 期初」两列建模 | ⬜ **未做**，且它可能反过来影响 `D-016` 的列绑定判据 |

⇒ **顺序是固定的**：不解决 1 就撞不到 2，不解决 2 就撞不到 3。

### C.1.1 🔴 第 2 层：我把它记成「25 个字段的数据工作」，那是没量过的假设

**量了之后**：25 个字段里 **21 个用茅台的变体就直接命中万华** ——
报表格式是规范化的，标签大面积相同。**只有 4 个不命中**，而且分成两类：

| 字段 | 差异 | 处置 |
|---|---|---|
| `bs.total_equity` | 万华写 `所有者权益（或股东权益）`，**无「合计」**，且不折行 | ✅ 补一条变体 |
| `is.net_profit` | 万华**同一行印完**，茅台折行 | ✅ 补一条变体 |
| `is.net_profit_attributable_to_parent` | **折行形态不同**，见 C.1.2 | ⬜ 要新的缝合模式 |
| `cfs.cash_paid_for_fixed_intangible_and_other_long_term_assets` | 同上 | ⬜ 同上 |

⚠️ **补 `bs.total_equity` 那条变体时有一处真陷阱**：万华同页上方还有一行
`所有者权益（或股东权益）：` —— **小节标题，与目标只差一个全角冒号**。
逐字整行相等把它挡在外面；**若改成子串或前缀匹配，取到的就是那个标题行（数值为空）**——
那正是 `A-4` 记的那次真实失手。⇒ 这两条是「多一种措辞」，**不是「放宽匹配」**。

**回归**：茅台 25 个字段仍然全命中，且**无一多命中**（多命中比零命中更危险 ——
它会静默取错行，而零命中至少会红）。

### C.1.2 ⬜ 第 2 层剩下的两个：折行形态不同，要新的缝合模式

万华的折行是「**标签 + 数值同行，续行只有标签**」：

```
购建固定资产、无形资产和其他长期    17,814,814,580.17  10,545,415,710.37
资产支付的现金
```

茅台的是「**标签行 / 纯数值行 / 标签行**」三连（`A-6` 失效模式 1 的原始形态）。
现有缝合器只认后者。

🔴 **那条规则量过了，是错的 —— 而我第一次给的理由也不对。**

第一次写的理由是「会把 `非流动负债：` 这类**小节标题**并进上一行」。
**实测：那只占 3–4 处。真正的问题大一个量级。**

在两家公司的三张报表上模拟「值行 + 纯标签行 ⇒ 合并」：
**万华会误并 48 处、茅台会误并 32 处**，绝大多数是这种：
...
⇒ **根因是「空行项目」**：标准报表模板里列着的、本公司没有金额的那些行
（`结算备付金` / `持有待售资产` / `向中央银行借款` / `质押贷款净增加额`…），
**本身就是纯标签行，一张表里有几十个**。它们与「续行」在版面上**完全同形**。
冒号判别式只挡得住小节标题，挡不住它们。

⚠️ **这条规则会连茅台一起弄坏** —— 现在跑通的单公司抽取会当场退化。

**为什么现有的三行缝合是安全的**：它要求**中间那行是纯数值行**，
而一个空行项目**不会产出纯数值行**。这个信号很强，这也是当初挑它的原因。

**往下走的方向（留给做这件事的人，不是本轮的结论）**：判别式必须能分开
「上一行标签的续行」与「一个空的行项目」。可能的信号是**续行的 x 起点相对标签起点的缩进**，
或者反过来 —— **由映射驱动的缝合**：只在某条已声明的两片段变体明确要求时才合并，
这样一个空行项目不会被并进去，除非有人真的写了一条把它俩连起来的变体。
⇒ 无论哪条，都要先在两家公司上量，**不许照着直觉改**。

### C.1.3 ✅ 成果：第二家公司算出了正确的财务比率

`bs` 那一路在万华 2019 上**端到端跑通**：18 条记录、**勾稽差额 0.00**、
`restated` 正确置位为 `True`，三个比率算出。

**独立可核**：万华 p73 印着 负债和所有者权益总计 `96,865,322,655.29`、
所有者权益 `43,931,258,971.63` ⇒ 负债 `52,934,063,683.66`
⇒ 资产负债率 = **0.5465**，与抽取器算出的一致。
⚠️ **这个期望值是从版面上两个数手算的，不是照抄抽取器的输出。**

⚠️ **但 `SC-2` 在万华上仍不成立**：`is` 与 `cfs` 各差一个字段（C.1.2 那两个），
且**没有人逐字段核对过万华的取值**（只核对了资产负债率这一个派生结果）。

顺丰控股 2021（`002352_2021.pdf`，282 页）**未跑三张报表**：它是深交所模板，
`restated` 与 `scope_change` 两节的形态都与上交所不同（见 §C.7 与台账 `N-38`），
先解决上交所内部的跨公司问题更有次序。
