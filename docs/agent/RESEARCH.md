...
**`INFERENCE`**：单人项目做通用版无法在功能上竞争，且回答不了"你和它的区别是什么"。
...
它承认**人必须复核**。本项目的主张正接在这句后面：**除非口径可被机械校验，否则复核成本高到没人真的会做。** 二者是互补而非竞争关系 —— 这是这个项目差异化最干净的一句话。
...
## 5. ~~岗位侧的现状~~ —— **本节已移出**（`D-044`，2026-09-11）

原本这一节是作者自己的岗位调研：具体公司的面试原题、岗位要求、AI 面覆盖情况。
那是**个人求职资料，不是这个项目的研究**，转公开时整节移出，
连同被点名的几家公司一起。本仓外 `../finaudit-workshop/` 留档。

⚠️ 移的是**这一节**，不是它的结论 —— 它当时支撑的那条判断
（「对数据准确性做校验」这件事在实务里被明确期待）已经落进 `IDEA.md` 的问题定义，
不依赖这一节存在。
