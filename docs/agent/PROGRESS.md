...
- 阶段：**Phase 1 已收口**；当前 **Phase 1.5 — 数据接入层**，
  **wave 1 / 2 / 3 已收口**（2026-08-27），wave 4–5 未开始
...
- 测试：**601 passed**；门禁 **6 道** + CI + 供应链，全 exit 0
- 指标定义：**20 / 20** 份 `version: 2`，全部合规；`advisory_only` 占比 19.0%（阈值 50%）
- 评测：frozen-01 冻结（21 文件哈希）；对照臂两个模型跑完
- **阻塞项**：A-2（有息负债率拒答，修在抽取层）。A-7 / A-8 已裁决（D-021 / D-020）
- **待操作者裁决**：`N-38` 判据 ③（`.md` 控制字符门禁，存量已清零）。
  `N-39` 与 `N-38` 判据 ② 已于 2026-08-27 裁决（`D-028` / `D-020` 补充节）；
  X 区**操作者已定：暂时放空**
