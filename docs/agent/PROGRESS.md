...
- 阶段：**Phase 1 已收口**；当前 **Phase 1.5 — 数据接入层**，
  **wave 1 / 2 / 3 / 4 已收口**（wave 4 收于 2026-08-28），**wave 5 未开始**
  （Phase 0 — cninfo 实证结论：未开始，可并行）
- **跨源对照已接通**：24 个字段可比 20 / 不可比 4 / 不一致 0（茅台 2023）。
  ⚠️ 差额全为 0 ⇒ **这批数据无法用来标定阈值**；`DISAGREES` 真实数据上零样本
- **SC-2 已达成**：30 字段中 29 取到值并逐个人工核对正确，1 条按 `D-026` 第 1 类作废
  （作废率 3.33% ≤ 10%）。⚠️ **单公司单年**；那 16 个是 wave 1 的结论，wave 2 未复核
- 已执行方法论阶段：DISCOVER ✅ / RESEARCH ✅ / ARCHITECT ✅ / POC-01 ✅（PASS，压线）
  / PLAN ✅ / IMPLEMENT ✅（Phase 1）/ VERIFY ✅（Phase 1）
- 测试：**671 passed**；门禁 **6 道** + CI + 供应链，全 exit 0
- 指标定义：**20 / 20** 份 `version: 2`，全部合规；`advisory_only` 占比 19.0%（阈值 50%）
- 评测：frozen-01 冻结（21 文件哈希）；对照臂两个模型跑完
- **阻塞项：无。** ~~A-2（有息负债率拒答，修在抽取层）~~ **判据已达成**（wave 3，`5770185`）：
  `tests/test_missing_semantics.py` 两个方向各有真实茅台数据驱动的回归
  （`test_有息负债率在真实批次上算得出数值而不是拒答` 得 `0.0011869876` /
  `test_有息负债率在整行不存在的批次上拒答而不是给零`），
  且 `test_定义侧一个字都没改` 锁住定义侧未动。A-7 / A-8 已裁决（D-021 / D-020）
- **待操作者裁决**：~~`N-38` 判据 ③（`.md` 控制字符门禁）~~ **已落地**
  （`1ca5872`，`tests/test_conformance.py::test_被跟踪的md里没有字面控制字符`，存量先清零后上门禁）。
  当前只剩三条**非阻塞**的：`N-20` 补做（只读 YAML 验证定义对人可读，执行者代答无效）、
  **供应链门槛为 akshare 由 10M 下调到 3M 是否接受**、X 区是否维持放空
  （操作者 2026-08-27 已定「暂时放空」）
  ⚠️ 上面这三行是 2026-08-28 校准的 —— **它们此前分别停在 wave 3 前与 08-27 的状态**，
  是 `N-35` 那个「做了但没回标」的又两个实例
