...
**已随 `references/` 一并移出本仓（`D-043`，2026-09-11）。** 那个目录是作者自己的
外部阅读笔记，不是这个项目的产物，转公开时不上传；R5 的检查对象随之不存在。
⚠️ **这是一次真的减法，不是「暂时关掉」** —— 放弃了什么、什么条件下拿回来，
逐条写在 `D-043`，不在这里重复。
...
- **抓不到「断言了，但断言的不是它声称证明的事」。** 这是本项目从三个外部项目
  收集到的那条模式（四个实例，笔记不在本仓）。断言内容的正确性无法机械判定。
...
NOT_GATES: dict[str, str] = {
    ...
    "check_h2_selfcontained.py": (
        "H2 题包的自足性检查。⚠️ **措辞要准**：它**会**在发现缺口时退非零 —— "
        "那是重出题包之后给人跑的一道，**不是**提交链上的第八环。"
        "链上的判定点是 `tests/test_h2_selfcontained.py`（把 G1/G2/G3 与出处那一节"
        "分别挖掉看红，另有一条把判据与 `answer._shown` 钉在一起）。"
        "🔴 **它证明的是「材料齐了」，不是「AC-06 通过」** —— 脚本自己的 docstring "
        "与输出里都写着这一句，且有测试守着那句话在不在"
    ),
