...
## ~~第五道门：阅读台账~~ —— **已随 `references/` 移出本仓**（`D-043`，2026-09-11）

脚本与它的测试现在在 `../finaudit-workshop/`，要手动跑：

```bash
python ../finaudit-workshop/check_reading_ledger.py
```

🔴 **它已经不在提交链上了。** 从此「台账标了『全文』而正文零对应内容」
这件事**没有任何自动检查会发现** —— 而那是实际发生过四次的失效模式。
放弃了什么、什么条件下拿回来，逐条在 `D-043`。**不要把这里读成「还守着」。**
