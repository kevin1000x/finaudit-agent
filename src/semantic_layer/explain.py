...
3. **本定义声明过的字段标识符不出现在正文；机器那一版集中放到末尾的核对附录。**
   ...
   🔴 **这一条此前写的是「正文里不出现字段标识符」，那句话不成立**
   （2026-09-05 独立复核指出，实测确认）。成立的是上面这句**加了限定**的版本。
   剩下的一类换不动：定义作者写「禁止取 X」时，X 按定义**不在** `source_fields` 里，
   没有中文名可换，编一个出来读者就没法发现禁的到底是哪一个。
   **实测残留：9 / 20 份定义、13 处、8 个不同字段**，全部属于这一类
   （`tests/test_explain.py::test_正文里剩下的字段标识符有多少_这个数被钉住` 逐条 assert
   并把这个数钉住，两个方向都会红）。
   真正的修法在**定义文件的散文本身**，不在渲染器 —— 记 `N-58`，要操作者裁。
