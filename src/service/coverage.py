...
## 真实数据从哪来（`N-61`，2026-09-06 收口）

`data/extracted/*.yaml` 是 `python -m extractor export` 的产物：
跑一次抽取器，**只导出人工逐字段核对过的字段**，带 `kind: real`
与年报 PDF 的 SHA-256。本模块只读它，不自己去解 PDF。

⚠️ **覆盖面的上限是「核对过的」，不是「抽得出的」。**
磁盘上有三份年报 PDF，但只有两家逐字段核对过，所以只有两家在清单里。

## 🔴 「真实 / 合成」这条线由**文件自己**说了算，不由目录说了算

按目录贴标签是 `N-42` 的形状：往 `data/extracted/` 放一份合成文件，
它就会被标成年报原文，而页面上没有一处露得出来。
⇒ 判据在 `_scan()`：`meta.kind == "real"` **且**给得出 64 位 PDF 指纹。
认不出就当合成 —— 把真实说成合成只是保守，反过来是这个项目最不能犯的错（`D-010`）。

## ⚠️ 本模块的字符串会被**原样印在纯文本证据页与网页上**

所以里面不许写 Markdown 强调号。写了的话 `**我们没有**` 会带着星号印出来
（2026-09-06 浏览器实跑撞到过一次）。`tests/test_service.py` 有一条守着。
"""
