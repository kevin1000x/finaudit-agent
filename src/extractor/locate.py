...
def strip_invisible(text: str) -> str:
    ...
    该报 p250 的章节标题，`extract_words` 取出来是 `'五\x07、合并范围的变更（此处 \x07 是转义写法，不是真的字符）'`
...
def _stitch(raw: list[tuple[str, tuple[Cell, ...], int, float]]) -> list[ReconstructedRow]:
    ...
    ## 为什么中间那行必须是**纯数值行**（2026-08-31 实测补记，别放宽它）

    万华化学 600309 · 2019 用的是**另一种折行**：`标签 + 数值同行`，续行只有标签 ——
    ...
    看起来只要再加一条「值行 + 纯标签行 ⇒ 合并」就能覆盖。**量过了，那条规则是错的**：
    在两家公司的三张报表上模拟，**万华会误并 48 处、茅台会误并 32 处**，例如
    `货币资金` + `结算备付金`、`存货` + `持有待售资产`、`短期借款` + `向中央银行借款`。

    ⇒ **根因是「空行项目」**：标准报表模板里列着、而本公司没有金额的那些行，
    **本身就是纯标签行，一张表里有几十个**，与「续行」在版面上**完全同形**。
    「续行不以全角冒号结尾」这类判别式只挡得住小节标题（3–4 处），挡不住它们。

    ⚠️ **而现有这条之所以安全，正是因为它要求中间是纯数值行 ——
    一个空行项目不会产出纯数值行。** 放宽这个条件会当场弄坏已经跑通的样本。
    完整测量见 `VERIFICATION.md` §C.1.2。
    """
